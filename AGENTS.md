...
- **Required secret accessors live in `tracecat/auth/secrets.py`** — use `get_db_encryption_key()`, `get_signing_secret()`, `get_service_key()`, `get_user_auth_secret()` at the point of use. Each returns `str` and raises `ValueError` if unset. No startup validation — the getter IS the check. The 4 raw values (`config.USER_AUTH_SECRET`, `config.TRACECAT__DB_ENCRYPTION_KEY`, `config.TRACECAT__SIGNING_SECRET`, `config.TRACECAT__SERVICE_KEY`) default to `None` in `config.py`. Do NOT default secrets to `""` — an empty string is worse than `None` because it silently passes through crypto APIs (jwt, fernet, hmac) as a guessable key. `USER_AUTH_SECRET` is required for all auth types (used by `UserManager` for password reset and email verification token signing) but API-only — do NOT provision it to executor, worker, or MCP services.
