...
- **Required secrets are validated once at startup** — `config.validate_required_secrets()` checks the 3 base secrets (`TRACECAT__DB_ENCRYPTION_KEY`, `TRACECAT__SIGNING_SECRET`, `TRACECAT__SERVICE_KEY`) needed by all services; `config.validate_api_secrets()` additionally checks `USER_AUTH_SECRET` (only the API needs it — do NOT provision it to executor, worker, or MCP). All 4 default to `None` in `config.py`. Do NOT add per-call-site `if not: raise` checks — use the typed accessors `config.require_db_encryption_key()`, `config.require_signing_secret()`, `config.require_service_key()`, `config.require_user_auth_secret()` where a `str` (not `str | None`) is needed. Do NOT default secrets to `""` — an empty string is worse than `None` because it silently passes through crypto APIs (jwt, fernet, hmac) as a guessable key. New service entrypoints must call `validate_required_secrets()` (or `validate_api_secrets()` for API) at startup.
...
- PR titles *MUST* also follow conventional commits format (e.g., `feat(agent): add streaming support`, `fix(api): handle null workspace ID`). Keep the title under 72 characters.
