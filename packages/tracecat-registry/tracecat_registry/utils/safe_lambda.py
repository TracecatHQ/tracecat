"""Safe lambda expression builder for registry actions.

Copied from tracecat/sandbox/safe_lambda.py to allow registry actions
...
def _sandbox_lambda(func: Callable[[Any], Any]) -> Callable[[Any], Any]:
    ...
    def sandboxed_wrapper(x: Any) -> Any:
...
def build_safe_lambda(lambda_expr: str) -> Callable[[Any], Any]:
    ...
    Args:
        lambda_expr: A string containing a lambda expression.

    Returns:
        A sandboxed callable that can be safely executed.

    Raises:
        ValueError: If the expression is invalid, too long, or contains
            dangerous patterns.
    """
    ...
    lambda_func = eval(code, restricted_globals, {})  # noqa: S307
