"""Add an opaque session backend identity independently of the harness.
...
def upgrade() -> None:
    op.add_column(
        "agent_session",
        sa.Column("backend_id", sa.String(50), server_default="v1", nullable=False),
    )
...
def downgrade() -> None:
    ...
    if connection.scalar(
        sa.text("SELECT EXISTS (SELECT 1 FROM agent_session WHERE backend_id <> 'v1')")
    ):
        raise RuntimeError("Cannot remove backend identity while non-v1 sessions exist")
