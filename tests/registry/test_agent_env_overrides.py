...
def patched_model_config(model_name: str, provider: str):
    ...
    def spy_get_model(name, prov, base_url=None):
    ...
    try:
        mod.get_model = spy_get_model  # type: ignore
        yield spy
    finally:
        mod.get_model = original_get_model  # type: ignore
...
@pytest.mark.anyio
async def test_run_agent_merges_env_fixed_arguments_and_injects_correctly(monkeypatch, mocker):
    ...
    mock_ra1 = Mock(); mock_ra1.namespace = "core.cases"; mock_ra1.name = "create_case"
    mock_ra2 = Mock(); mock_ra2.namespace = "tools.slack"; mock_ra2.name = "post_message"
    ...
    mock_ctx = AsyncMock(); mock_ctx.__aenter__.return_value = mock_service
    mocker.patch.object(bmod.RegistryActionsService, "with_session", return_value=mock_ctx)
