...
# =============================================================================
# Test Class 10: PYTHONPATH Isolation
# =============================================================================


class TestPythonpathIsolation:
    """Tests for actual PYTHONPATH isolation between tenants.

    Verifies that:
    - Each request receives the correct PYTHONPATH
    - Different tenants get different PYTHONPATHs
    - Same worker correctly switches PYTHONPATH between requests
    """

    @pytest.mark.anyio
    async def test_pythonpath_returned_in_test_mode_response(
        self,
        sandboxed_pool: SandboxedWorkerPool,
        run_action_input_factory: Callable[..., RunActionInput],
        role_workspace_a: Role,
        ...
        """Verify PYTHONPATH is correctly forwarded and returned in response.

        In test mode, the worker returns the pythonpath it received.
        This verifies the path is correctly forwarded through the pool.

        Validates:
        - Worker receives the pythonpath parameter
        - Response includes the correct pythonpath
        """
        path_a, _ = staged_cache_dirs
    ...
    @pytest.mark.anyio
    async def test_different_tenants_receive_different_pythonpaths(
        self,
        sandboxed_pool,
        run_action_input_factory,
        role_workspace_a: Role,
        ...
        """Verify different tenants receive their respective PYTHONPATHs.

        Each workspace should receive only its own PYTHONPATH, not
        another tenant's path.

        Validates:
        - Workspace A receives path_a
        - Workspace B receives path_b
        - No cross-contamination
        """
        path_a, path_b = staged_cache_dirs

        # Execute for workspace A
        result_a = await sandboxed_pool.execute(
            input=run_action_input_factory(),
            role=role_workspace_a,
            timeout=30.0,
        )

        # Execute for workspace B
        result_b = await sandboxed_pool.execute(
            input=run_action_input_factory(),
            role=role_workspace_b,
            timeout=30.0,
        )
        ...
        assert pythonpath_b == str(path_b), (
            f"Workspace B should receive path_b: {pythonpath_b}"
        )
        assert pythonpath_a != pythonpath_b, "Tenants should have different PYTHONPATHs"

    @pytest.mark.anyio
    async def test_workspace_id_correctly_forwarded(
        self,
        sandboxed_pool: SandboxedWorkerPool,
        run_action_input_factory: Callable[..., RunActionInput],
        role_workspace_a: Role,
        ...
        """Verify workspace_id is correctly forwarded to workers.

        In test mode, the worker returns the workspace_id from the role.
        This verifies tenant identity is maintained.

        Validates:
        - Worker receives correct workspace_id
        - Different tenants have different workspace_ids
        """
        result_a = await sandboxed_pool.execute(
            input=run_action_input_factory(),
            role=role_workspace_a,
        ...
        assert workspace_id_a != workspace_id_b, (
            f"Workspaces should have different IDs: {workspace_id_a} vs {workspace_id_b}"
        )

    @pytest.mark.anyio
    async def test_single_worker_switches_pythonpath_correctly(
        self,
        single_worker_pool: SandboxedWorkerPool,
        run_action_input_factory: Callable[..., RunActionInput],
        role_workspace_a: Role,
        ...
        """Verify single worker correctly switches PYTHONPATH between requests.

        With a single worker, all requests go to the same worker.
        This tests that PYTHONPATH is correctly switched per-request.

        Validates:
        - Same worker handles both tenants
        - PYTHONPATH changes correctly per request
        - No path leakage between sequential requests
        """
        pool = single_worker_pool
        path_a, path_b = staged_cache_dirs
        ...
        for i in range(10):
            if i % 2 == 0:
                role = role_workspace_a
                expected_path = str(path_a)
            else:
                role = role_workspace_b
                expected_path = str(path_b)
            ...
            assert received_path == expected_path, (
                f"Request {i}: Expected {expected_path}, got {received_path}"
            )

        # Verify pattern: A, B, A, B, A, B, A, B, A, B
        for i, (expected, received) in enumerate(received_paths):
            assert expected == received, f"Request {i} path mismatch"

    @pytest.mark.anyio
    async def test_concurrent_requests_maintain_path_isolation(
        self,
        run_action_input_factory: Callable[..., RunActionInput],
        role_workspace_a: Role,
        role_workspace_b: Role,
        ...
        """Verify concurrent requests from different tenants maintain isolation.

        When requests from multiple tenants run concurrently, each should
        receive only its own PYTHONPATH.

        Note: This test uses its own pool to ensure clean state.
        ...
        try:
            ...
            # Interleave and execute
            all_tasks = tasks_a + tasks_b
            expected_paths = [t[0] for t in all_tasks]
            coros = [t[1] for t in all_tasks]

            results = await asyncio.gather(*coros)

            # Verify each result matches its expected path
            mismatches = []
            ...
            assert len(mismatches) == 0, "Path isolation failures:\n" + "\n".join(
                mismatches
            )
        finally:
            await pool.shutdown()
