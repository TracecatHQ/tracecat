...
async def test_run_action_payload_too_large(
    ...
    large_data = "x" * (TRACECAT__EXECUTOR_PAYLOAD_MAX_SIZE_BYTES + 1000)
    large_result = {"result": large_data}

    # Ensure it actually exceeds the limit when serialized
    assert len(orjson.dumps(large_result)) > TRACECAT__EXECUTOR_PAYLOAD_MAX_SIZE_BYTES
    ...
    with patch("tracecat.executor.router.dispatch_action_on_cluster") as mock_dispatch:
        ...
        assert (
            f"exceeds the size limit of {TRACECAT__EXECUTOR_PAYLOAD_MAX_SIZE_BYTES / 1000}KB"
...
async def test_run_action_payload_exactly_at_limit(
    ...
    chars_to_add = TRACECAT__EXECUTOR_PAYLOAD_MAX_SIZE_BYTES - template_overhead
    exact_sized_result = {"result": "x" * chars_to_add}

    # Confirm it's exactly at the limit
    serialized = orjson.dumps(exact_sized_result)
    assert len(serialized) == TRACECAT__EXECUTOR_PAYLOAD_MAX_SIZE_BYTES
...
async def test_run_action_payload_size_just_over_limit(
    ...
    chars_to_add = TRACECAT__EXECUTOR_PAYLOAD_MAX_SIZE_BYTES - template_overhead + 1
    ...
    assert len(serialized) > TRACECAT__EXECUTOR_PAYLOAD_MAX_SIZE_BYTES
    assert (
        len(serialized) <= TRACECAT__EXECUTOR_PAYLOAD_MAX_SIZE_BYTES + 10
    )  # Should be just over
