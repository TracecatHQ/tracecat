...
async def test_executor_can_run_template_action_with_oauth(
        ...
        definition=TemplateActionDefinition(
            ...
            steps=[
                ActionStep(
                    ...
                    args={
                        "value": {
                            "oauth_token": "${{ OAUTH.microsoft_teams.USER_TOKEN }}",
                            "secret_token": "${{ SECRETS.microsoft_teams.MICROSOFT_TEAMS_USER_TOKEN }}",
                            "tokens_equal": "${{ OAUTH.microsoft_teams.USER_TOKEN == SECRETS.microsoft_teams.MICROSOFT_TEAMS_USER_TOKEN }}",
                        }
                    },
                )
            ],
    ...
    # which contains oauth_token, secret_token, and tokens_equal fields
    assert isinstance(result, dict), f"Expected dict result, got {type(result)}"
    assert "oauth_token" in result, (
        f"Expected 'oauth_token' in result, got {result.keys()}"
    )
    assert "secret_token" in result, (
        f"Expected 'secret_token' in result, got {result.keys()}"
    )
    assert "tokens_equal" in result, (
        f"Expected 'tokens_equal' in result, got {result.keys()}"
    )

    # Verify the values
    assert result["secret_token"] == oauth_token_value, (
        f"OAuth token from SECRETS namespace mismatch. "
        f"Expected {oauth_token_value}, got {result['secret_token']}"
    )
    assert result["oauth_token"] == oauth_token_value, (
        f"OAuth token from OAUTH namespace mismatch. "
        f"Expected {oauth_token_value}, got {result['oauth_token']}"
    )
    assert result["tokens_equal"] is True, (
        "OAUTH.microsoft_teams.USER_TOKEN should equal "
        "SECRETS.microsoft_teams.MICROSOFT_TEAMS_USER_TOKEN"
    )
