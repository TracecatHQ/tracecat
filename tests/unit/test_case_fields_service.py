...
class TestCaseFieldsService:
    ...
    async def test_get_fields(
        ...
        # Mock the editor.get_row method
        mock_fields_data = {
            "id": case_fields.id,
    ...
    async def test_insert_field_values(
        self, case_fields_service: CaseFieldsService, test_case: Case
    ) -> None:
        ...
        # Mock result from editor.insert_row
        mock_result = {
            "id": uuid.uuid4(),
            "case_id": test_case.id,
            "custom_field1": "test value",
        ...
        # Mock the editor.insert_row method
        with patch.object(case_fields_service.editor, "insert_row") as mock_insert_row:
            ...
            # Verify the table row insert contained the correct data
            call_args = mock_insert_row.call_args[0][0]
            assert isinstance(call_args, TableRowInsert)
            assert call_args.data["case_id"] == test_case.id
