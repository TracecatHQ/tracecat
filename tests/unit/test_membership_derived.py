...
async def test_any_role_path_is_organization_presence(
    session: AsyncSession,
    org: Organization,
    workspace: Workspace,
    user: User,
    ...
    """A workspace-scoped path alone is organization presence."""
    session.add(
        UserRoleAssignment(
            organization_id=org.id,
            user_id=user.id,
    ...
    # A group's org-wide grant adds org presence with no workspace row.
