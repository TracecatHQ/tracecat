...
class TestAgentPresetService:
    ...
    async def test_version_reads_use_normalized_subagent_binding(
        ...
        """Detail reads use normalized membership and current ResourceHeads."""
        ...
        assert detail_ref.preset_version_id == normalized_child_version_two.id
        assert detail_ref.preset_version == normalized_child_version_two.version
    ...
    async def test_create_preset_locks_skill_bindings_during_validation(
        ...
        assert captured_for_update == [True, True]
    ...
    async def test_version_binding_resolves_migrated_slug_only_versions(
        self,
        session: AsyncSession,
        agent_preset_service: AgentPresetService,
        agent_preset_create_params: AgentPresetCreate,
    ) -> None:
        """Head-edge resolution must not depend on migrated legacy JSON.
        ...
        cannot validate as ``ResolvedAgentsConfig``. With edges present,
        resolution reads only the normalized edges and never revalidates the
        legacy JSON.
        """
