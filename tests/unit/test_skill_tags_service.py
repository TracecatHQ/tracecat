...
from tracecat.db.models import SkillTag, SkillTagLink
...
def _skill_tag(name: str, created_at: datetime, workspace_id: object) -> SkillTag:
    return SkillTag(
        id=uuid4(),
...
@pytest.mark.parametrize(
    ...
    [
        ...
        lambda service: service.list_tags_for_skill_paginated(
            uuid4(), CursorPaginationParams()
        ),
        lambda service: service.get_tag(uuid4()),
    ],
)
async def test_skill_tag_read_methods_require_agent_read_scope(
    invoker: Callable[[SkillTagsService], Awaitable[object]],
) -> None:
    """Read methods should reject callers without agent:read before querying."""
    session = AsyncMock()
    service = SkillTagsService(session=session, role=_role_with_scopes(frozenset()))
...
@pytest.mark.parametrize(
    ...
    [
        (
            lambda service: service.list_tags_for_skill(uuid4()),
        ...
        (
            lambda service: service.get_skill_tag(uuid4(), uuid4()),
            frozenset({"agent:update"}),
        ),
        (
            lambda service: service.add_skill_tag(uuid4(), uuid4()),
            frozenset({"agent:update"}),
        ),
        (
            lambda service: service.remove_skill_tag(object()),
            frozenset({"agent:update"}),
        ),
    ],
)
async def test_skill_tag_methods_require_agent_addons_entitlement(
    invoker: Callable[[SkillTagsService], Awaitable[object]],
    role_scopes: frozenset[str],
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Skill-tag association methods should preserve the AGENT_ADDONS gate."""
    session = AsyncMock()
    service = SkillTagsService(session=session, role=_role_with_scopes(role_scopes))
...
async def test_skill_tag_definition_methods_require_agent_addons_entitlement(
    invoker: Callable[[SkillTagsService], Awaitable[object]],
    role_scopes: frozenset[str],
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Skill tag definition CRUD should preserve the AGENT_ADDONS gate."""
    session = AsyncMock()
    service = SkillTagsService(session=session, role=_role_with_scopes(role_scopes))
    mock_has_entitlement = AsyncMock(return_value=False)
    monkeypatch.setattr(service, "has_entitlement", mock_has_entitlement)
...
async def test_list_tags_paginated_reverse_pages_use_canonical_order(
    ...
    raw_reverse_rows = [
        _skill_tag("older", base_time + timedelta(minutes=1), workspace_id),
        _skill_tag("middle", base_time + timedelta(minutes=2), workspace_id),
        _skill_tag("newer", base_time + timedelta(minutes=3), workspace_id),
    ]
    result = MagicMock()
    result.scalars.return_value.all.return_value = raw_reverse_rows
    session = AsyncMock()
    session.execute.return_value = result
    service = SkillTagsService(
        session=session,
...
@pytest.mark.anyio
async def test_list_tags_for_skill_paginated_uses_cursor_pages(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Paginated skill tag listing should return stable bounded pages."""
    workspace_id = uuid4()
    skill_id = uuid4()
    base_time = datetime(2026, 1, 1, tzinfo=UTC)
    rows = [
    ...
    page = await service.list_tags_for_skill_paginated(
        skill_id, CursorPaginationParams(limit=2)
    )
...
@pytest.mark.anyio
async def test_list_tags_for_skill_paginated_invalid_cursor_raises_validation(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Invalid skill tag cursors should surface app-level validation errors."""
    service = SkillTagsService(
        session=AsyncMock(),
        role=_role_with_scopes(frozenset({"agent:read"})),
    )
    monkeypatch.setattr(service, "has_entitlement", AsyncMock(return_value=True))
    monkeypatch.setattr(service, "_require_skill_in_workspace", AsyncMock())

    with pytest.raises(TracecatValidationError, match="Invalid cursor for skill tags"):
        await service.list_tags_for_skill_paginated(
...
async def test_get_tag_missing_raises_tracecat_not_found(
    ...
    with pytest.raises(TracecatNotFoundError, match="Skill tag not found"):
...
async def test_update_tag_commit_conflict_raises_tracecat_conflict(
    ...
    workspace_id = uuid4()
    tag = _skill_tag("alpha", datetime(2026, 1, 1, tzinfo=UTC), workspace_id)
    result = MagicMock()
    result.one_or_none.return_value = None
    session = AsyncMock()
    session.execute.return_value = result
    session.commit.side_effect = IntegrityError(
        statement="UPDATE skill_tag",
        params={},
    ...
    with pytest.raises(
        TracecatConflictError, match="Skill tag with slug 'beta' already exists"
    ):
...
@pytest.mark.anyio
async def test_list_tags_for_skill_requires_existing_skill(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Skill tag listing should fail when the skill is missing."""
    session = AsyncMock()
    ...
    with pytest.raises(TracecatNotFoundError, match="Skill not found"):
        await service.list_tags_for_skill(uuid4())

    session.execute.assert_not_awaited()


@pytest.mark.anyio
async def test_add_skill_tag_returns_existing_link_for_duplicate_assignment(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Duplicate skill-tag assignments should return the existing link."""
    existing_link = SkillTagLink(tag_id=uuid4(), skill_id=uuid4())
    ...
    link = await service.add_skill_tag(existing_link.skill_id, existing_link.tag_id)
