...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            {"user.name": "Alice", "user.tags[0]": "admin", "user.tags[1]": "user"},
        ),
        ...
        # Mixed types
        (
            {"str": "hello", "num": 42, "bool": True, "null": None},
            {"str": "hello", "num": 42, "bool": True, "null": None},
        ),
        ...
        (
            ...
            {
                "data.users[0].id": 1,
                "data.users[0].tags[0]": "admin",
                "data.users[1].id": 2,
                "data.users[1].tags[0]": "user",
                "data.users[1].tags[1]": "guest",
            },
        ),
    ],
)
def test_flatten_dict(input_data: dict | list, expected: dict) -> None:
    """Test flatten_dict function with various nested structures."""
    assert flatten_dict(input_data) == expected


def test_flatten_dict_max_depth_default() -> None:
    """Test that flatten_dict works with deep nesting within default limit."""
    # Create a deeply nested structure (50 levels)
    deep_obj: dict = {"value": "leaf"}
    for _ in range(50):
        deep_obj = {"level": deep_obj}

    # Should succeed with default max_depth=100
    result = flatten_dict(deep_obj)
    assert "level." * 50 + "value" in list(result.keys())[0]
    assert result[list(result.keys())[0]] == "leaf"


def test_flatten_dict_max_depth_exceeded() -> None:
    """Test that flatten_dict raises ValueError when max_depth is exceeded."""
    ...
    with pytest.raises(ValueError, match="Maximum recursion depth \\(3\\) exceeded"):
        flatten_dict(deep_obj, max_depth=3)


def test_flatten_dict_max_depth_custom() -> None:
    """Test flatten_dict with custom max_depth that allows operation."""
    # Create a moderately nested structure (3 levels)
    data = {"a": {"b": {"c": 1}}}

    # Should succeed with max_depth=5
    result = flatten_dict(data, max_depth=5)
    assert result == {"a.b.c": 1}

    # Should fail with max_depth=1
    with pytest.raises(ValueError, match="Maximum recursion depth \\(1\\) exceeded"):
        flatten_dict(data, max_depth=1)


def test_flatten_dict_max_depth_with_arrays() -> None:
    """Test max_depth protection with arrays."""
    # Nested arrays within objects
    data = {"level1": [{"level2": [{"level3": [{"value": 1}]}]}]}

    # Should succeed with sufficient depth
    result = flatten_dict(data, max_depth=10)
    assert result == {"level1[0].level2[0].level3[0].value": 1}

    # Should fail with insufficient depth
    with pytest.raises(ValueError, match="Maximum recursion depth \\(2\\) exceeded"):
