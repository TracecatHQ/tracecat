...
@pytest.mark.anyio
async def test_refresh_token_replay_within_grace_uses_rotated_upstream_token(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """A replayed pre-rotation token succeeds without replaying upstream."""
    auth, client = _build_test_auth_and_client(monkeypatch)
    monkeypatch.setattr(mcp_config, "TRACECAT_MCP__REFRESH_REUSE_GRACE_SECONDS", 3600)
    client_id = _register_public_client(client)
    oauth_client = _mock_upstream_refresh_client(["rt-2", "rt-3"])
    monkeypatch.setattr(auth, "_create_upstream_oauth_client", lambda: oauth_client)
    old_refresh = await _seed_refresh_session(auth, client_id=client_id)

    first = _post_refresh_token(client, client_id=client_id, refresh_token=old_refresh)
    replay = _post_refresh_token(client, client_id=client_id, refresh_token=old_refresh)

    assert first.status_code == 200
    assert replay.status_code == 200
    assert replay.json()["refresh_token"] != first.json()["refresh_token"]
    # The replay refreshed with the CURRENT upstream refresh token (rt-2), so
    # the internal issuer saw two normal rotations and never a replay.
    assert [
        call.kwargs["refresh_token"]
        for call in oauth_client.refresh_token.await_args_list
    ] == ["rt-1", "rt-2"]


@pytest.mark.anyio
async def test_refresh_token_replay_after_grace_deadline_fails(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    auth, client = _build_test_auth_and_client(monkeypatch)
    monkeypatch.setattr(mcp_config, "TRACECAT_MCP__REFRESH_REUSE_GRACE_SECONDS", 60)
    client_id = _register_public_client(client)
    oauth_client = _mock_upstream_refresh_client(["rt-2", "rt-3"])
    monkeypatch.setattr(auth, "_create_upstream_oauth_client", lambda: oauth_client)
    old_refresh = await _seed_refresh_session(auth, client_id=client_id)
    ...
    # fastmcp maps invalid_grant to HTTP 401 per the MCP spec.
    assert replay.status_code == 401
    assert replay.json()["error"] == "invalid_grant"
    assert oauth_client.refresh_token.await_count == 1


@pytest.mark.anyio
async def test_refresh_token_zero_grace_enforces_single_use_rotation(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Grace 0 keeps strict one-time-use rotation (Claude Code regression)."""
    auth, client = _build_test_auth_and_client(monkeypatch)
    monkeypatch.setattr(mcp_config, "TRACECAT_MCP__REFRESH_REUSE_GRACE_SECONDS", 0)
    client_id = _register_public_client(client)
    oauth_client = _mock_upstream_refresh_client(["rt-2", "rt-3"])
    monkeypatch.setattr(auth, "_create_upstream_oauth_client", lambda: oauth_client)
    old_refresh = await _seed_refresh_session(auth, client_id=client_id)
    ...
    # The rotated token still works — the normal rotation chain is intact.
    rotated = _post_refresh_token(
        client, client_id=client_id, refresh_token=first.json()["refresh_token"]
    )
    assert rotated.status_code == 200


@pytest.mark.anyio
async def test_refresh_token_replay_grace_deadline_does_not_slide(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    auth, client = _build_test_auth_and_client(monkeypatch)
    monkeypatch.setattr(mcp_config, "TRACECAT_MCP__REFRESH_REUSE_GRACE_SECONDS", 100)
    client_id = _register_public_client(client)
    oauth_client = _mock_upstream_refresh_client(["rt-2", "rt-3", "rt-4"])
    monkeypatch.setattr(auth, "_create_upstream_oauth_client", lambda: oauth_client)
    old_refresh = await _seed_refresh_session(auth, client_id=client_id)
    old_hash = _hash_token(old_refresh)

    base_now = time.time()
    monkeypatch.setattr(time, "time", lambda: base_now)
    first = _post_refresh_token(client, client_id=client_id, refresh_token=old_refresh)
    assert first.status_code == 200

    resurrected = await auth._refresh_token_store.get(key=old_hash)
    assert resurrected is not None
    original_deadline = resurrected.expires_at
    assert original_deadline == int(base_now + 100)

    monkeypatch.setattr(time, "time", lambda: base_now + 50)
    replay = _post_refresh_token(client, client_id=client_id, refresh_token=old_refresh)
    assert replay.status_code == 200

    resurrected_again = await auth._refresh_token_store.get(key=old_hash)
    assert resurrected_again is not None
    # The replay must keep the ORIGINAL deadline, not extend it to now+grace.
    assert resurrected_again.expires_at == original_deadline
