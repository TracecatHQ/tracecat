...
@pytest.mark.anyio
async def test_exec_pod(kubernetes_repo, mock_validate_access, mock_secret_service):
    """Test the kubernetes exec pod template action."""

    # Get the registered action
    bound_action = kubernetes_repo.get("ee.kubernetes.exec")

    # Test exec pod
    test_args = {"pod": "test-pod", "command": "sleep 3600", "namespace": "test-ns"}
...
@pytest.mark.anyio
async def test_logs(kubernetes_repo, mock_validate_access, mock_secret_service):
    """Test the kubernetes logs template action."""

    # Get the registered action
    bound_action = kubernetes_repo.get("ee.kubernetes.logs")

    # Test logs
    test_args = {"pod": "test-pod", "namespace": "test-ns"}
