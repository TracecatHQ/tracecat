...
class TestMCPProviderOAuth:
    ...
    async def test_templated_catalog_uri_rejects_foreign_host(
        self,
        integration_service: IntegrationService,
        session: AsyncSession,
        monkeypatch: pytest.MonkeyPatch,
    ) -> None:
        """A region placeholder does not let the row bind to an arbitrary host."""
        await _seed_service_user(session, integration_service)
        catalog = _catalog_entry(
            slug="secops-region-mcp",
            name="SecOps Region MCP",
            description="Regional vendor endpoint",
            connection_spec={
                "kind": "http_oauth2",
                "server_type": "http",
                "auth_type": "OAUTH2",
        ...
        def params(server_uri: str) -> MCPHttpIntegrationCreate:
        ...
        for foreign in (
            ...
            with pytest.raises(ValueError, match="host does not match"):
                await integration_service.create_mcp_integration(params=params(foreign))
        ...
        # A real region still binds, and the pinned endpoints are used.
        result = await integration_service.connect_mcp_oauth_discovery(
            params=params("https://chronicle.eu.rep.googleapis.com/mcp")
        )
        assert result.oauth_connect is not None
        assert result.mcp_integration is not None
        assert (
            result.mcp_integration.server_uri
