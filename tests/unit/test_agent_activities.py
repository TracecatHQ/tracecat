...
class TestCreateSessionActivity:
    ...
    async def test_creates_new_session(
        self, mock_role: Role, mock_session_id: uuid.UUID
    ):
        ...
        mock_service.get_or_create_session = AsyncMock(
            return_value=(MagicMock(), True)  # (session, created=True)
        )

        # Create a proper async context manager mock
        mock_ctx_manager = MagicMock()
        mock_ctx_manager.__aenter__ = AsyncMock(return_value=mock_db_session)
        mock_ctx_manager.__aexit__ = AsyncMock(return_value=None)

        with (
            patch(
                "tracecat.agent.session.activities.get_async_session_context_manager",
                return_value=mock_ctx_manager,
            ),
            patch(
                "tracecat.agent.session.activities.AgentSessionService"
            ) as mock_service_cls,
        ):
    ...
    async def test_idempotent_for_existing_session(
        ...
        mock_service = MagicMock()
        mock_service.get_or_create_session = AsyncMock(
            return_value=(MagicMock(), False)  # (session, created=False)
        )
    ...
    async def test_sets_correct_harness_type(
        ...
        mock_db_session.commit = AsyncMock()
        mock_service = MagicMock()
        mock_service.get_or_create_session = AsyncMock(return_value=(MagicMock(), True))
...
class TestLoadSessionActivity:
    ...
    async def test_loads_session_without_history(
        ...
        mock_service = MagicMock()
        mock_service.get_session = AsyncMock(return_value=mock_agent_session)
        mock_service.load_session_history = AsyncMock(return_value=None)
    ...
    async def test_loads_session_with_history(
        ...
        mock_service = MagicMock()
        mock_service.get_session = AsyncMock(return_value=mock_agent_session)
        mock_service.load_session_history = AsyncMock(return_value=mock_history)
