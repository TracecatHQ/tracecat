...
# =============================================================================
# Test Class 5: Pool Worker with Different PYTHONPATHs
# =============================================================================


class TestPoolWorkerPythonpath:
    """Tests for pool worker PYTHONPATH handling.

    Verifies that:
    - Same worker handles requests with different PYTHONPATHs sequentially
    - PYTHONPATH is passed correctly per-request
    - Pool correctly receives and forwards pythonpath parameter
    """

    @pytest.mark.anyio
    async def test_pythonpath_passed_to_worker_socket(
        self, mock_run_action_input_factory, role_workspace_a: Role
    ):
        """Verify that the pool passes pythonpath through to worker execution.

        When pool.execute() is called with a pythonpath parameter, it must be
        forwarded to _execute_on_worker() unchanged. The worker uses this to
        set PYTHONPATH before importing registry modules.

        Validates:
        - pythonpath parameter reaches _execute_on_worker() intact
        """
        ...
        captured_request: dict[str, str | None] = {}

        async def mock_execute_on_worker(
            worker: SandboxedWorkerInfo,
            ...
            captured_request["pythonpath"] = pythonpath
            return {"success": True, "result": {}}

        pythonpath = "/tmp/registry-v1:/tmp/registry-custom"
        input_data = mock_run_action_input_factory()

        with patch.object(pool, "_execute_on_worker", mock_execute_on_worker):
            await pool.execute(
                input=input_data,
        ...
        assert captured_request["pythonpath"] == pythonpath

    @pytest.mark.anyio
    async def test_sequential_requests_different_pythonpaths(
        self, mock_run_action_input_factory, role_workspace_a: Role
    ):
        """Verify that a worker can handle sequential requests with varying PYTHONPATHs.

        A single pool worker may process requests for different workspaces
        sequentially. Each request carries its own pythonpath, and the worker
        must apply the correct one for each request.

        This ensures workers are stateless with respect to PYTHONPATH - they
        don't cache or reuse it across requests.

        Validates:
        - Three sequential requests with different paths are captured correctly
        - Each request's pythonpath is passed through independently
        """
        pool = SandboxedWorkerPool(size=1)
        pool._started = True
        ...
        async def mock_execute_on_worker(
            ...
            captured_paths.append(pythonpath)
            return {"success": True, "result": {}}

        with patch.object(pool, "_execute_on_worker", mock_execute_on_worker):
            # Request 1 with path A
            await pool.execute(
                input=mock_run_action_input_factory(),
                role=role_workspace_a,
                timeout=60.0,
            )

            # Request 2 with path B
            await pool.execute(
                input=mock_run_action_input_factory(),
                role=role_workspace_a,
                timeout=60.0,
        ...
        assert captured_paths == [
            "/path/to/registry-v1",
            "/path/to/registry-v2",
            "/path/to/custom-registry",
        ]

    @pytest.mark.anyio
    async def test_pythonpath_none_handled_correctly(
        self, mock_run_action_input_factory, role_workspace_a: Role
    ):
        """Verify that None pythonpath is handled gracefully.

        When TRACECAT__LOCAL_REPOSITORY_ENABLED is True or no registry artifacts
        exist, pythonpath may be None. The pool must pass this through correctly
        so the worker knows not to modify PYTHONPATH.

        Validates:
        - None pythonpath is forwarded to _execute_on_worker() as None (not empty string)
        """
        pool = SandboxedWorkerPool(size=1)
        pool._started = True
        ...
        captured_pythonpath: dict[str, str | None] = {}
        ...
        async def mock_execute_on_worker(
            ...
            captured_pythonpath["value"] = pythonpath
            return {"success": True, "result": {}}
        ...
        assert captured_pythonpath["value"] is None
