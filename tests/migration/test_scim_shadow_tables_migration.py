...
def test_upgrade_creates_shadow_tables_and_columns(migration_db: str) -> None:
    """The three shadow tables and both new columns land together."""
    engine = _engine(migration_db)
    try:
        with engine.connect() as conn:
            for name in NEW_TABLES:
                assert _table_exists(conn, name), f"{name} was not created"
            assert _column_exists(conn, "group_member", "source")
    finally:
        engine.dispose()


def test_existing_group_members_backfill_to_manual(previous_db: str) -> None:
    """Rows written by the old app version become manual, never scim."""
    ...
    try:
        with engine.begin() as conn:
            ...
            # The old writer knows nothing about a source column.
            conn.execute(
                text("INSERT INTO group_member (user_id, group_id) VALUES (:u, :g)"),
        ...
        with engine.connect() as conn:
            source = conn.execute(
                text("SELECT source FROM group_member WHERE user_id = :u"),
                {"u": user_id},
            ).scalar_one()
...
def test_old_writer_still_inserts_group_members(migration_db: str) -> None:
    """The server default keeps the previous app version writing unchanged."""
    engine = _engine(migration_db)
    try:
        with engine.begin() as conn:
            org_id = _seed_org(conn)
...
def _seed_external_user(
    ...
    conn.execute(
        text(
            ...
            "VALUES (gen_random_uuid(), :org, :u, :ext)"
        ),
        {"org": org_id, "u": user_id, "ext": external_id},
    )
...
def test_deleting_external_group_cascades_to_members_and_mappings(
    ...
    try:
        with engine.begin() as conn:
            ...
            conn.execute(
                text(
                    "INSERT INTO external_group_member (external_group_id, user_id) "
                    "VALUES (:eg, :u)"
                ),
                {"eg": external_id, "u": user_id},
            )
            conn.execute(
                text(
...
def test_downgrade_removes_everything_the_upgrade_added(migration_db: str) -> None:
    ...
    try:
        ...
        with engine.connect() as conn:
            ...
            assert not _column_exists(conn, "group_member", "source")
            assert (
                conn.execute(
                    text("SELECT 1 FROM pg_type WHERE typname = 'groupmembersource'")
                ).scalar_one_or_none()
                is None
            )
