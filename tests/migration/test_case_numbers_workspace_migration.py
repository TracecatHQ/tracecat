...
def test_case_number_migration_recovers_from_existing_trigger(
    migration_db_url: str,
) -> None:
    workspace_a_id, _ = _seed_pre_migration_cases(migration_db_url)
    engine = create_engine(migration_db_url, poolclass=NullPool)
    ...
    try:
        ...
        with engine.begin() as conn:
            workspace_counter = conn.execute(
                text(
                    """
                    SELECT last_case_number
                    FROM workspace
