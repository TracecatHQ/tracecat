...
class AuthorizationService:
    ...
    def __init__(self, session: AsyncSession):
        ...
        self.logger = logger.bind(service="authz")

    @asynccontextmanager
    @staticmethod
    async def with_session(
        role: Role | None = None,
    ) -> AsyncGenerator[AuthorizationService, None, None]:
        async with get_async_session_context_manager() as session:
            yield AuthorizationService(session, role=role)

    async def add_resource_owner(
        self, resource_id: str, owner_id: UUID4, resource_type: str, owner_type: str
    ) -> None:
    ...
    async def user_is_workspace_member(
        self, user_id: UUID4, workspace_id: UUID4
    ) -> bool:
    ...
    async def user_can_access_resource(self, user_id: UUID4, resource_id: str) -> bool:
