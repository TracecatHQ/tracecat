...
async def run_action_from_input(input: RunActionInput, role: Role) -> Any:
    """Main entrypoint for running an action directly in-process.

    This function is used by DirectBackend for testing. It handles full
    context setup including secrets resolution, registry context, and
    logical time.
    """
    ctx_role.set(role)
    ...
    # Note: Masking is applied by the caller (run_action_on_cluster) after execution
    # to ensure uniform masking across all backends
