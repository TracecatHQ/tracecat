...
class DSLScheduler:
    ...
    def __init__(
        self,
        *,
        executor: Callable[[ActionStatement], Awaitable[ExecuteTaskResult]],
        dsl: DSLInput,
    ...
    async def _schedule_task(self, task: Task) -> None:
        ...
        try:
            ...
            # XXX(activity): RUN INSIDE ACTIVITY
            # ==================================================================
    ...
    async def _handle_gather(
        ...
        match task:
            ...
            case Task(stream_id=stream_id) if stream_id and stream_id.endswith(":skip"):
                ...
                if self._task_observed(scatter_task):
                    ...
                    parent_action_context[task.ref] = ActionSuccess(
                        result=InlineObject(
                            data=TaskResult(result=[], result_typename="list")
                        )
                    )
                else:
                    self.logger.debug(
        ...
        if gather_ref not in parent_action_context:
            ...
            parent_action_context[gather_ref] = ActionSuccess(
                result=InlineObject(
                    data=TaskResult(
                        result=result, result_typename=type(result).__name__
                    )
                )
            )

        cast(InlineObject, parent_action_context[gather_ref]).data["result"][
            stream_idx
        ] = items
    ...
    async def _handle_gather_skip_stream(
        ...
        if err_info := self.stream_exceptions.get(stream_id):
            ...
            cast(InlineObject, parent_action_context[gather_ref]).data["result"][
                stream_idx
            ] = InlineObject(data=err_info.details)
    ...
    async def _handle_gather_result(
        ...
        parent_action_context: ActionContext,
        ...
        stored_object = parent_action_context.setdefault(
            gather_ref,
            ActionSuccess(
                result=InlineObject(
                    data=TaskResult(result=[], result_typename=list.__name__)
                )
            ),
        )
        # Scheduler only creates InlineObject internally; externalization happens at boundaries
        assert isinstance(stored_object, ActionSuccess)
        result = stored_object.result
    ...
    def _get_action_context(self, stream_id: StreamID) -> ActionContext:
    ...
    def build_stream_aware_context(
        ...
        return ExecutionContext(actions=resolved_actions)
    ...
    def get_stream_aware_action_result(
        ...
        while curr_stream is not None:
            ...
            if stream_context := self.streams.get(curr_stream):
                actions_context = stream_context.actions
