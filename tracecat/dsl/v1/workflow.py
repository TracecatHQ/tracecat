...
from tracecat.dsl.v1 import materializer
from tracecat.dsl.v1.scheduler import DSLScheduler
from tracecat.dsl.v1.types import ExecuteTaskResult, ExecutionContext
...
class DSLWorkflowV1:
    ...
    async def _setup_workflow(self, args: DSLRunArgs) -> None:
        ...
        self.context = ExecutionContext(
            actions={},
            trigger=InlineObject(data=trigger_inputs) if trigger_inputs else None,
            env=DSLEnvironment(
                workflow={
                    "start_time": wf_info.start_time,
    ...
    async def run(self, args: DSLRunArgs) -> Any:
        await self._setup_workflow(args)
    ...
    async def _execute_task(self, task: ActionStatement) -> ExecuteTaskResult:
    ...
    async def _execute_child_workflow(
        ...
        if task.for_each:
            ...
            materialized_context = materializer.materialize(self.get_context())
            args = evaluate_templated_args(task, context=materialized_context)
            self.logger.trace(
    ...
    async def _execute_child_workflow_loop(
        ...
        *,
        ...
        def iterator() -> Generator[ExecuteChildWorkflowArgs]:
            for args in iter_for_each(task=task, context=materialized_context):
    ...
    def _handle_return(self) -> Any:
        ...
        if self.dsl.returns is None:
            match config.TRACECAT__WORKFLOW_RETURN_STRATEGY:
                case "context":
                    ...
                    materialized_context.pop(ExprContext.ENV, None)
        ...
        self._set_logical_time_context()
        return eval_templated_object(self.dsl.returns, operand=materialized_context)
    ...
    async def _resolve_workflow_alias(self, wf_alias: str) -> identifiers.WorkflowID:
        ...
        self._set_logical_time_context()
        materialized_context = materializer.materialize(self.get_context())
        evaluated_alias = eval_templated_object(wf_alias, operand=materialized_context)
    ...
    async def _noop_gather_action(self, task: ActionStatement) -> Any:
        ...
        arg = RunActionInput[ExecutionContext](
            task=task,
    ...
    async def _run_action(self, task: ActionStatement) -> Any:
        ...
        stream_id = ctx_stream_id.get()
        # Inside the activity we would materialize the context to a dict[ExprContext, Any]
        new_context = self._build_action_context(task, stream_id)
        materialized_context = materializer.materialize(new_context)

        # Inject current logical_time into the workflow context for FN.now() etc.
        if env_context := new_context.env:
        ...
        if task.environment is not None:
            ...
            self._set_logical_time_context()
            evaluated_env = eval_templated_object(
                task.environment, operand=materialized_context
            )
