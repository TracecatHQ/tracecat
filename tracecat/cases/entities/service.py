...
from tracecat.db.schemas import CaseEntityLink, EntityData, EntityMetadata
...
class CaseEntitiesService(BaseWorkspaceService):
    ...
    async def list_entities_for_case(
        ...
        Args:
            case_id: The case ID to list entities for
    ...
    async def create_and_associate_entity(
        self,
        case_id: uuid.UUID,
        entity_metadata_id: uuid.UUID,
        ...
        # Associate with case
        link = await self.associate_entity_with_case(
            case_id=case_id,
    ...
    async def get_entity_metadata(
        self, entity_metadata_id: uuid.UUID
    ) -> EntityMetadata | None:
        ...
        stmt = select(EntityMetadata).where(
            EntityMetadata.id == entity_metadata_id,
            EntityMetadata.owner_id == self.workspace_id,
        )
        result = await self.session.exec(stmt)
        return result.one_or_none()

    async def list_available_entity_types(self) -> Sequence[EntityMetadata]:
        """List all available entity types in the workspace.

        Returns:
            List of EntityMetadata records
        """
        stmt = select(EntityMetadata).where(
            EntityMetadata.owner_id == self.workspace_id,
