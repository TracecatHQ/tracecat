"""Data models and adapter logic for the Vercel AI SDK.
...
class VercelStreamContext:
    """State machine that converts unified agent events into AI SDK SSE frames.
    ...
    def _create_part_state(
        ...
        if tool_call is not None:
            self.tool_index[tool_call.id] = index
        return state
            ...
            if not self.tool_input_emitted.get(tool_call_id, False):
                events.append(
                    ToolInputAvailableEventPayload(
                        ...
                        toolName=state.tool_call.name,
                        input=state.tool_call.input,
                    )
                )
        ...
        match event.type:
            ...
            case StreamEventType.TOOL_CALL_START:
                ...
                tool_call = ToolCallContent(
                    id=tool_call_id,
                    name=tool_name,
                    input=strip_proxy_tool_metadata(event.tool_input or {}),
                )
            ...
            case StreamEventType.TOOL_CALL_DELTA:
                if event.part_id is not None:
                    ...
                    if state and state.tool_call and event.text:
                        if state.tool_call.name not in AGENT_TOOL_NAMES:
                            yield ToolInputDeltaEventPayload(
                                toolCallId=state.tool_call.id,
                ...
                if event.part_id is not None:
                    ...
                    if state and state.tool_call:
                        ...
                        if not self.tool_input_emitted.get(tool_call_id, False):
                            ...
                            tool_name = event.tool_name or state.tool_call.name
                            tool_input = sanitize_agent_tool_input(
                                tool_name,
                                event.tool_input or state.tool_call.input,
                            )
            ...
            case StreamEventType.APPROVAL_REQUEST:
                # Unified approval request from the durable runtime.
            ...
            if isinstance(block, ToolUseBlock):
                approvals.append(
                    ToolCallContent(
                        id=block.id,
                        name=block.name,
                        input=strip_proxy_tool_metadata(block.input or {}),
                    )
                )
        ...
        if approval_payload:
            mutable_message.parts.append(
                DataUIPart(
                    type=APPROVAL_DATA_PART_TYPE,
                    data=[
                        _approval_item_to_data_part(item) for item in approval_payload
                    ],
                )
            )
