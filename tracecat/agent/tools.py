...
async def create_tool_from_registry(
    ...
    *,
    ...
    parameters_json_schema = (
        model_cls.model_json_schema() if hasattr(model_cls, "model_json_schema") else {}
    )
...
class SimpleToolExecutor(ToolExecutor):
    """Simple tool executor that runs actions via subprocess."""

    async def run(self, tool_name: str, args: dict[str, Any]) -> Any:
        """Execute a tool by canonical name.

        Args:
            tool_name: Canonical action name (e.g., "core.http.request")
            args: Arguments to pass to the action
        ...
        """
        logger.info("Executing tool call", tool_name=tool_name)
        return await call_tracecat_action(tool_name, args)
