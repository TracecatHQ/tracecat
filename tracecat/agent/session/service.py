...
ACTIVE_AGENT_SESSION_STATUSES = ("running", "waiting_for_approval")
...
class AgentSessionService(BaseWorkspaceService):
    ...
    @staticmethod
    def _status_after_missing_current_run(
        current_status: AgentSessionStatus,
    ) -> AgentSessionStatus:
        """Keep terminal UI state if present; otherwise unblock as idle."""
        if current_status.value in TERMINAL_AGENT_SESSION_STATUSES:
    ...
    async def _has_active_agent_turn(self, agent_session: AgentSession) -> bool:
        ...
        try:
            ...
            if e.status != RPCStatusCode.NOT_FOUND:
                raise
            agent_session.status = self._status_after_missing_current_run(
                AgentSessionStatus(agent_session.status)
            ).value
    ...
    async def validate_turn_request(
        ...
        match request:
            ...
            case VercelChatRequest() | BasicChatRequest():
                if await self._has_active_agent_turn(agent_session):
                    raise TracecatConflictError(
