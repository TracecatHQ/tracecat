...
from tracecat.agent.sandbox.protocol import RuntimeInitPayload, RuntimeInitPayloadTA
...
class ClaudeAgentRuntime(AgentRuntime):
    ...
    Key differences from legacy runtimes:
    - No ChatService or DBMessageStore access
    - No direct call_tracecat_action() - uses MCP proxy
    - Events streamed via socket, not Redis
    - Session data received via socket, written locally for SDK resume
    """
...
async def _run_sandboxed_runtime(socket_path: str) -> None:
    """Entry point for sandboxed runtime execution.

    Connects to orchestrator socket, receives init payload, runs agent.
    """
    ...
    try:
        ...
        logger.exception("Runtime error", error=e)
...
def main() -> None:
    ...
    asyncio.run(_run_sandboxed_runtime(args.socket))
