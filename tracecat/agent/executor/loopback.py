...
from tracecat.agent.sandbox.socket_io import MessageType, build_message, read_message
from tracecat.agent.stream.connector import AgentStream
from tracecat.agent.stream.types import StreamEventType, ToolCallContent
...
class LoopbackInput:
    ...
    - user_prompt, config, jwt_token, litellm_*, allowed_actions, sdk_session_*
...
class LoopbackHandler:
    ...
    async def _send_init_payload(self, writer: asyncio.StreamWriter) -> None:
        ...
        # The MCP socket directory is mounted at /mcp-sockets inside the NSJail sandbox
        # so we send the jailed path, not the host path

        payload = RuntimeInitPayload(
            session_id=self.input.session_id,
            jwt_token=self.input.jwt_token,
            config=self.input.config,
            user_prompt=self.input.user_prompt,
            litellm_base_url=self.input.litellm_base_url,
        ...
        payload_bytes = orjson.dumps(asdict(payload), default=to_jsonable_python)
    ...
    async def _process_runtime_events(self, reader: asyncio.StreamReader) -> None:
        ...
        while True:
            ...
            # Parse the envelope
            envelope = RuntimeEventEnvelopeTA.validate_json(payload_bytes)
                    ...
                    if envelope.event:
                        ...
                        if envelope.event.type == StreamEventType.APPROVAL_REQUEST:
                            ...
                            self._result.approval_items = (
                                envelope.event.approval_items or []
                            )
