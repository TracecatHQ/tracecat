...
from tracecat.agent.sandbox.exceptions import AgentSandboxExecutionError
    ...
    if result.success:
        ...
        try:
            async with get_async_session_context_manager() as session:
                svc = AgentSessionService(session=session, role=input.role)
...
async def execute_approved_tools_activity(
    ...
    if results:
        ...
        async with get_async_session_context_manager() as db_session:
            session_service = AgentSessionService(db_session, input.role)
