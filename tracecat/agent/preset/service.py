...
class AgentPresetService(BaseWorkspaceService):
    ...
    async def update_preset(
        ...
        current_agents = await self._get_version_agents_config(current_version)
        current_skill_ids = await self._get_version_skill_binding_ids(
            current_version.id
        )
        ...
        elif "model_name" in execution_updates or "model_provider" in execution_updates:
        ...
        current_execution = AgentPresetExecutionConfigWrite.model_validate(
            {
        ...
        execution = AgentPresetExecutionConfigWrite.model_validate(
            current_execution.model_dump() | execution_updates
        )
        ...
        execution_changed = (
            ...
            or "agents" in execution_updates
        )
        self.session.add(preset)
    ...
    async def _count_head_subagent_references(self, preset: AgentPreset) -> int:
        ...
        legacy_count = 0
        for agents_json in (await self.session.scalars(legacy_stmt)).all():
            agents = AgentSubagentsConfig.model_validate(agents_json)
            if any(
                getattr(ref, "preset_id", None) == preset.id
                or (
                    getattr(ref, "preset_id", None) is None
                    and ref.preset == preset.slug
                )
    ...
    async def _resolve_preset_subagent_configs(
        ...
        *,
        ...
        # never a silent drop.
        if resolved.skipped and not allow_skipped:
            skipped_refs = sorted(
                skipped.preset_slug or str(skipped.preset_id)
                for skipped in resolved.skipped
            )
            raise TracecatValidationError(
                "Cannot save preset because it references unavailable subagent "
    ...
    async def _version_subagent_diff_value(
        ...
        """Return a stable, JSON-ready representation of version subagent edges."""
        agents = await self._get_version_agents_config(version)
        return [
            {
                "preset_id": str(subagent.preset_id),
