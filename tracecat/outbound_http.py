...
type SyncHostResolver = Callable[[str, int], tuple[SocketInfo, ...]]
...
class GuardedNetworkBackend(httpcore.NetworkBackend):
    """Resolve, validate, and pin each synchronous TCP connection."""
    ...
    def __init__(
        ...
        *,
        resolver: SyncHostResolver = resolve_host,
        backend: httpcore.NetworkBackend | None = None,
    ) -> None:
        self._policy = policy
        self._request_origin = request_origin
        self._resolver = resolver
        self._backend = backend or cast(httpcore.NetworkBackend, httpcore.SyncBackend())
...
class GuardedHTTPTransport(httpx.HTTPTransport):
    """HTTPX transport whose synchronous connections use validated DNS."""
...
def guarded_client(
    ...
    *,
    ...
    """Create a synchronous guarded client with proxy routing disabled."""
