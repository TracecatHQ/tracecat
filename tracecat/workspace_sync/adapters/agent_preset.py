...
class AgentPresetAdapter(DirectoryManifestAdapter):
    ...
    async def _version_specs_for_preset(
        ...
        *,
        ...
        for version in version_rows:
            ...
            normalized_subagents = subagent_bindings_by_version_id.get(version.id)
            subagents = normalized_subagents
            if subagents is None:
                subagents = (
                    _legacy_subagent_refs_for_unreconciled_version(version) or []
                )
    ...
    async def _subagent_bindings_for_versions(
        ...
        for (
            ...
            if deleted_at is not None and source_id is None:
                # Preserve a stable identity that cannot collide with an active
                # preset reusing the tombstone's slug. Import then fails closed
                # unless that identity can be mapped in the target workspace.
    ...
    async def import_specs(
        ...
        for source_id, spec in sorted(presets.items()):
            ...
            if preset is None:
                preset = AgentPreset(
                    ...
                    agents=ResolvedAgentsConfig().model_dump(mode="json"),
                )
            else:
        ...
        for source_id in import_order:
            ...
            if head_spec is not None:
                ...
                resolved_head_agents = await self._resolved_subagents_config(
                    workspace_service, head_spec
                )
            else:
                resolved_head_agents = ResolvedAgentsConfig()
            preset.agents = resolved_head_agents.model_dump(mode="json")
            workspace_service.session.add(preset)
            await workspace_service.session.flush()
            await self._replace_head_subagent_bindings(
                workspace_service,
                preset,
                resolved_head_agents.to_head_config(),
            )
            skill_targets = (
            ...
            for version_number, version_spec in sorted(version_specs.items()):
                (
                    version,
                    resolved_version_agents,
                ) = await self._upsert_agent_preset_version(
                    workspace_service,
                    preset=preset,
                ...
                await self._replace_version_subagent_bindings(
                    workspace_service,
                    version,
                    resolved_version_agents.to_head_config(),
                )
            ...
            # Advance the preset to the imported current version.
    ...
    async def _resolved_subagents_config(
        ...
        """Build an exact-version compatibility config for ``spec``.
        ...
        for subagent in spec.subagents:
            ...
            if target is None:
                continue
            child, child_version = target
            subagents.append(
                ResolvedAttachedSubagentRef(
                    preset=child.slug,
                    preset_id=child.id,
                    # Resolved-shape compat payload: the JSON remains the
                    # mixed-version contract, and both old pods' strict ref
                    # union and the head dual-write's ResolvedAgentsConfig
                    # validation require version identifiers. Edges stay
                    # authoritative; these are the child's current version.
        ...
        return ResolvedAgentsConfig(enabled=bool(subagents), subagents=subagents)
    ...
    async def _resolved_subagent_target(
        ...
        """Resolve a subagent ref to its live child head and current version."""
        ...
        if child is None:
            return None
        child_version = await self._current_version_for_preset(workspace_service, child)
    ...
    async def _current_version_for_preset(
        ...
        """Return ``preset``'s current :class:`AgentPresetVersion`, if published."""
    ...
    async def _create_agent_preset_version(
        ...
        """
        resolved_agents = ResolvedAgentsConfig.model_validate(preset.agents)
        ...
        await self._replace_version_subagent_bindings(
            workspace_service,
            version,
            resolved_agents.to_head_config(),
        )
        return version
    ...
    async def _upsert_agent_preset_version(
        ...
        *,
    ...
    ) -> tuple[AgentPresetVersion, ResolvedAgentsConfig]:
        ...
        resolved_agents = await self._resolved_subagents_config(
            workspace_service,
            version,
        )
        attrs = self._version_attrs_from_spec(version)
        attrs["agents"] = resolved_agents.model_dump(mode="json")
        if existing is None:
        ...
        await workspace_service.session.flush()
        return existing, resolved_agents
    ...
    async def _replace_head_subagent_bindings(
        ...
        workspace_service: SyncMappingService,
        preset: AgentPreset,
        agents: HeadAgentsConfig,
    ) -> None:
        """Replace normalized subagent ResourceHead edges for a preset head."""

        config = HeadAgentsConfig.model_validate(agents)
        preset.subagents_enabled = config.enabled
        await workspace_service.session.execute(
    ...
    async def _replace_version_subagent_bindings(
        self,
        workspace_service: SyncMappingService,
        version: AgentPresetVersion,
        agents: HeadAgentsConfig,
    ) -> None:
        """Replace normalized subagent ResourceHead edges for a version ledger row."""

        config = HeadAgentsConfig.model_validate(agents)
        version.subagents_enabled = config.enabled
        await workspace_service.session.execute(
...
def _legacy_subagent_refs_for_unreconciled_version(
    version: AgentPresetVersion,
) -> list[AgentPresetSubagentRef] | None:
    """Return legacy refs only when a version still needs edge reconciliation.

    ``None`` preserves an authoritative empty normalized snapshot. Once any
    normalized edge exists, callers never reach this compatibility fallback.
    """
    try:
        config = AgentSubagentsConfig.model_validate(version.agents)
    except ValidationError:
        return None
    if config.enabled == version.subagents_enabled and not config.subagents:
        return None
    return [
        AgentPresetSubagentRef(
