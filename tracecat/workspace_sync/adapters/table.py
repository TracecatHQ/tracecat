...
class TableAdapter(DirectoryManifestAdapter):
    ...
    async def import_specs(
        ...
        async def rename_table(table: Table, name: str) -> None:
            await table_service.update_table(table, TableUpdate(name=name))

        swap = await self.plan_name_swap(
            workspace_service,
            targets={source_id: spec.name for source_id, spec in tables.items()},
            model=Table,
        ...
        for source_id, spec in sorted(tables.items()):
            ...
            table = swap.mapped_by_source_id.get(
                source_id
            ) or await self._table_by_source_id(
                workspace_service,
                source_id,
            )
