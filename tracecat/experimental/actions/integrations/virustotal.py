...
def create_virustotal_client() -> httpx.AsyncClient:
    return httpx.AsyncClient(base_url=VT_BASE_URL)


@registry.register(
    namespace="virustotal",
    description="Get file report by hash",
    secrets=["virustotal"],
)
async def get_file_report(file_hash: str) -> dict[str, Any]:
...
@registry.register(
    namespace="virustotal",
    description="Get URL analysis report by URL",
    secrets=["virustotal"],
)
...
async def get_ip_address_report(ip: str) -> dict[str, Any]:
