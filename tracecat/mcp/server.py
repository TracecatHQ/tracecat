...
async def update_case_trigger(
    ...
    `tag_filters`, so an omitted field is cleared rather than preserved. Read
    the current trigger with `get_case_trigger` first and pass all three back.
    Patching `/case_trigger` through `edit_workflow` is partial, unlike this
    tool.
