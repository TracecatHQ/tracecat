---
name: docs-pr-screenshots
description: Use when adding or updating documentation pages that need to match an existing docs style and ship with screenshots. Covers inspecting nearby docs for tone and structure, running product services or docs previews, capturing UI screenshots with Chrome DevTools, suppressing Next.js floating dev indicators before screenshots, taking full-height Mintlify docs screenshots for PR descriptions, keeping PR-only screenshots out of committed files, and creating or updating GitHub PRs with verified screenshot links.
---

# Docs PR Screenshots

Use this workflow for documentation PRs where the docs page, embedded screenshots, and PR description screenshots all need to look polished and consistent with the existing project.
...
3. Start the product stack needed to capture real UI screenshots. If the repo has cluster guidance, check for an existing stack before starting a new one.
4. Capture product screenshots through Chrome DevTools. Do not use OS screenshots when the user asked for Chrome DevTools.
5. Hide Next.js dev UI before every capture. Re-run the suppression after navigation, reloads, or hot updates.
6. Consider an env-driven Next.js dev UI suppression path if the project already supports one, or if the user approves adding a local dev-only switch.
7. Add only documentation content screenshots to the repo. Save PR-only full-page docs screenshots in a local ignored artifact folder or `/tmp`, never in the commit unless the user explicitly asks.
8. Run the docs preview with `mint`, capture full-height screenshots of every added or materially changed docs page, and upload or link those screenshots in the PR description.
9. Verify the docs, git diff, screenshot links, and PR body before calling the task done.
...
If `mint` is unavailable, use the repo-approved package runner or ask before installing global tooling. Keep the server running until screenshots and visual checks are complete.
...
grep -qxF ".docs-pr-screenshots/" .git/info/exclude || printf "\n.docs-pr-screenshots/\n" >> .git/info/exclude

# Example output path:
# .docs-pr-screenshots/skills-docs-full-page.png
```

For disposable artifacts, use `/tmp` instead:

```bash
mkdir -p /tmp/docs-pr-screenshots
# Example output path:
# /tmp/docs-pr-screenshots/skills-docs-full-page.png
```
...
- Confirm the PR body renders image Markdown and contains the validation notes.
