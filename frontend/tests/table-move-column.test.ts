 ...
 * from the cell text: a short first column and a long second one.
 */
const DERIVED_TABLE = `
<table>
  <tbody>
...
/** Percentage widths the node view has written onto the rendered `<col>`s. */
function renderedColumnWidths(element: HTMLElement): string[] {
  return Array.from(element.querySelectorAll("col")).map(
    (col) => col.style.width
  )
}
...
describe("derived widths across a move", () => {
  it("carries the derived proportions with the moved column", () => {
    withEditor(DERIVED_TABLE, (editor, element) => {
      ...
      expect(cellTexts(editor)[0]).toEqual(["n".repeat(80), "Host"])
      // The wide column stays wide now that it sits on the left.
      expect(renderedColumnWidths(element)).toEqual(["86.96%", "13.04%"])
    })
  })

  it("leaves the proportions alone while a cell is edited", () => {
    withEditor(DERIVED_TABLE, (editor, element) => {
      expect(renderedColumnWidths(element)).toEqual(["13.04%", "86.96%"])

      placeCursorInCell(editor, 1, 0)
      editor.commands.insertContent("x".repeat(60))
