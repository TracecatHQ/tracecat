...
/**
 * Renders a workflow trigger section scoped to a case.
 * Allows selecting a workflow and triggering it with that case's data.
 */
export function TaskWorkflowTrigger({
  ...
  const selectedWorkflowUrl = `/workspaces/${workspaceId}/workflows/${workflowId}`
  const { data: selectedWorkflowDetail } = useQuery<
    WorkflowWithSchema | null,
    ApiError
  >({
    enabled: Boolean(workflowId && workspaceId),
  ...
  const handleSchemaSubmit = useCallback(
    ...
    [createExecution, showExecutionStartedToast, workflowId]
  )
  ...
  const handleTriggerWithoutSchema = useCallback(async () => {
    ...
    showExecutionStartedToast()
    setIsConfirmOpen(false)
  }, [createExecution, fallbackInputs, workflowId, showExecutionStartedToast])
