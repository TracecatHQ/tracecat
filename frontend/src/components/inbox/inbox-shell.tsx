...
/** Accesses the inbox chat state owned by InboxShell. */
export function useInboxChat() {
  const context = useContext(InboxChatContext)
  if (!context) {
    throw new Error("useInboxChat must be used within InboxShell")
  }
  return context
}

/** Renders the inbox content and chat panel inside the shared workspace shell. */
