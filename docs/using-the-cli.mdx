...
Tracecat's Command-Line Interface (CLI) allows you to manage workflows without the UI.
It's the fastest way to interact with Tracecat and is perfect for automating tasks.
...
`pipx` is a tool that lets you install and run Python applications in isolated environments. It's conceptually similar to `npm`.
