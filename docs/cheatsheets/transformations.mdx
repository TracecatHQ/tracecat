...
- The transform needs `x[?(...)][0].field` or `x[?(...)].field[0]`: the first errors and the second silently returns the wrong value.
...
{
  "day": "2026-08-24",
  "parsed": "2026-08-06T06:57:15+02:00",
  "local": "2026-08-06T00:57:15-04:00"
}
```
</CodeGroup>
...
Hard-referencing only the skipped branch's result silently yields `null`, so use `||` to read whichever branch actually ran. Skip propagates to a downstream action only when all of its dependencies were skipped.
...
```json Result
{
  ...
  "timestamps": ["1997-09-15T00:00:00", "2030-01-01T00:00:00"]
}
```
</CodeGroup>
