---
title: "Functions"
description: "Quick reference for every Tracecat expression function and operator, with signatures and examples for transforming data inside workflows and agents."
---

import FunctionsReference from "/snippets/functions-reference.mdx";

## Examples

Build a prompt or short string:

<CodeGroup>
```yaml Expression
prompt: ${{ FN.format("Investigate alert {0} with severity {1}", TRIGGER.alert_id, TRIGGER.severity) }}
```

```json Result
{
  "prompt": "Investigate alert al-123 with severity high"
}
```
</CodeGroup>

Time manipulation for alert search windows.
`FN.to_isoformat` keeps microseconds by default; pass `"seconds"` as the second argument to truncate for APIs that expect second precision:

<CodeGroup>
```yaml Expression
start_time: ${{ FN.to_isoformat(FN.now() - FN.minutes(15)) }}
end_time: ${{ FN.to_isoformat(FN.now(), "seconds") }}
```

```json Result
{
  "start_time": "2026-03-18T12:45:00.490979",
  "end_time": "2026-03-18T13:00:00"
}
```
</CodeGroup>

Deserialize or serialize JSON:

<CodeGroup>
```yaml Expression
parsed: ${{ FN.deserialize_json(TRIGGER.raw_json) }}
serialized: ${{ FN.serialize_json({"status": "ok", "count": 3}) }}
```

```json Result
{
  "parsed": {
    "status": "open",
    "count": 3
  },
  "serialized": "{\"status\":\"ok\",\"count\":3}"
}
```
</CodeGroup>

Format rows into a Markdown table:

<CodeGroup>
```yaml Expression
table: ${{ FN.to_markdown_table(ACTIONS.list_findings.result) }}
```

```json Result
{
  "table": "|id|severity|\n|-|-|\n|f-1|high|\n|f-2|medium|"
}
```
</CodeGroup>

Count or reshape a small value:

<CodeGroup>
```yaml Expression
count: ${{ FN.length(ACTIONS.gather_results.result) }}
unique_tags: ${{ FN.unique(TRIGGER.tags) }}
```

```json Result
{
  "count": 3,
  "unique_tags": ["auth", "critical"]
}
```
</CodeGroup>

Functions accept positional arguments only: signatures in the reference below show parameter names such as `timespec: str = auto`, but keyword arguments do not parse.

This page is generated from `tracecat/expressions/functions.py`.

<FunctionsReference />
