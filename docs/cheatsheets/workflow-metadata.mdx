...
| `${{ ENV.workflow.run_id }}` | UUID of this run attempt of the execution |
| `${{ ENV.workflow.trigger_type }}` | `manual`, `scheduled`, `webhook`, or `case` |
| `${{ ENV.workflow.logical_time }}` | Logical timestamp of the current action; `FN.now()` returns it so time-based expressions stay consistent across replays. Set when action inputs are built, so a `run_if` evaluated before any action has run sees `None` |
