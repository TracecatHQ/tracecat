...
| `jamf` | `JAMF_BASE_URL` `JAMF_CLIENT_ID` `JAMF_CLIENT_SECRET` | - |
...
| `okta` | `OKTA_API_TOKEN` | - |
| `optional_secret` | `REQUIRED_KEY1` `REQUIRED_KEY2` | `OPTIONAL_KEY1` |
...
| integrations.jamf | get_auth_token | `jamf` |
| integrations.jamf | list_computer_inventory | `jamf`, `ssl` |
| integrations.jamf | post_mdm_command | `jamf`, `ssl` |
...
| integrations.okta | get_user | `okta`, `ssl` |
| integrations.okta | get_user_by_email | `okta`, `ssl` |
| integrations.okta | list_user_events | `okta`, `ssl` |
| integrations.okta | reset_password | `okta`, `ssl` |
...
| integrations.sentinel_one | list_threats | `sentinel_one`, `ssl` |
...
| integrations.sentinel_one | update_threat_status | `sentinel_one`, `ssl` |
