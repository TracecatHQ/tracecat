...
Expression syntax resembles Python but is a separate grammar. Each form below fails with `Error parsing expression`:
...
Never nest `${{ }}` inside `${{ }}`: `${{ FN.length(${{ TRIGGER.tags }}) }}` fails with `No terminal matches '$' in the current parser context`. Write the composition directly as `${{ FN.length(TRIGGER.tags) }}`.
...
Quoted keys work everywhere brackets do: `TRIGGER["data"]` and `VARS.cfg["epss"]` both resolve. Unquoted keys work in some contexts and fail in others, so always quote:
...
The first segment after `SECRETS` or `VARS` must be a dot: `VARS["cfg"]` fails with `Expected one of: * ATTRIBUTE_PATH`. Write `VARS.cfg` first, then brackets for anything deeper.

Missing keys behave differently by context. `${{ TRIGGER["nope"] }}` returns `null`, while the same lookup on `VARS` raises:
...
Hyphenated keys in payloads are reachable with quoted brackets: `${{ TRIGGER["my-key"] }}` works. A hyphenated secret or variable name is not reachable at all: `SECRETS.my-secret.API_KEY` fails to parse, no bracket form exists for those contexts, and the only fix is renaming the secret or variable with underscores.
...
`[0]` after a JSONPath filter is absorbed into the path and applied to each matched value, so with matched dates like `"1997-09-15"` you index into the string itself:
...
Moving the index before the field also fails: `events[?(...)][0].eventDate` raises `Error trying to process rule "actions": 0`. Take the first element with `FN.at` instead:
...
To pass an empty value to an action, write it in plain YAML outside the expression: `value: {}`. To test for emptiness, use `FN.is_empty(x)`, `FN.length(x) == 0`, or `x == None`.
...
A ` #` fails silently: YAML treats it as a comment, so `title: ${{ FN.format("Alert #{0}", TRIGGER.alert_id) }}` truncates to the literal text `${{ FN.format("Alert`, which passes through unevaluated. Wrap the whole value in single quotes whenever the expression contains `:`, `#`, or quotes:

```yaml
message: '${{ FN.format("Alert: {0}", TRIGGER.alert_id) }}'
```

Use single outer quotes, not double; double quotes collide with the double quotes inside the expression.
...
Use the accessor function instead: `${{ FN.get_hour(FN.now()) }}` returns the hour.
...
Expressions do work inside `python_lambda` parameters. Tracecat substitutes the value as text before the lambda compiles, so write the expression as the Python literal it should become:
...
```python Result
lambda x: x['epss'] > 0.75
```
</CodeGroup>

Numbers, booleans, `None`, lists, and objects substitute as valid literals bare. Strings need single quotes around the expression: `'${{ VARS.cfg.severity }}'` becomes `'High'`, while the unquoted form becomes the bare name `High` and fails at call time with `NameError: name 'High' is not defined`.

A string value containing an apostrophe breaks the lambda with `Invalid syntax in expression: unterminated string literal`. When interpolated data can contain quotes, move the logic to `core.script.run_python`, which receives values through `inputs` instead of text substitution.
