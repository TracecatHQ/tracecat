...
Never nest `${{ }}` inside `${{ }}`: `${{ FN.length(${{ TRIGGER.tags }}) }}` fails with `No terminal matches '$' in the current parser context`. Write `${{ FN.length(TRIGGER.tags) }}`.
...
Expressions work inside `python_lambda` parameters. Tracecat substitutes the value as text before the lambda compiles, so write the expression as the Python literal it should become:
