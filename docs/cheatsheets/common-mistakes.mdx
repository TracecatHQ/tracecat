...
<CodeGroup>
```yaml Expression
first_date: ${{ ACTIONS.get_domain_record.result.events[?(@.eventAction == "registration")].eventDate[0] }}
```
...
<CodeGroup>
```yaml Expression
first_date: ${{ FN.at(ACTIONS.get_domain_record.result.events[?(@.eventAction == "registration")].eventDate, 0) }}
```
