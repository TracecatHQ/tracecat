...
nsjail is the boundary. It runs by default on full-isolation profiles and applies to tenant code, third-party packages, and generated output alike.

The sandbox exposes only the files and broker interfaces the run needs. General automation actions use their configured executor network policy instead.

Without nsjail, containment weakens per code path: `core.script.run_python` falls back to a PID namespace where the host permits one, with no filesystem or network boundary either way, custom registry actions run as plain subprocesses that inherit the executor environment, and agents run as plain subprocesses with a scrubbed environment. Run without nsjail only when you trust every custom action and dependency.

See [Self-hosted security](/self-hosting/security#isolation) for isolation levels, requirements, and how to enable and verify nsjail.

When nsjail is enabled, it provides these controls:
...
| Profile | Isolation level | Contract | Intended use |
| --- | --- | --- | --- |
| Tracecat Cloud | nsjail | Tracecat manages the secure agent profile, trusted brokers, network boundary, and sandbox lifecycle. | Managed production workloads. |
| Self-hosted Kubernetes | nsjail | The Helm chart enables nsjail by default and applies the required executor security context itself. | Production workloads that run untrusted code or agents. |
| AWS Fargate | None | Fargate lacks the kernel capabilities nsjail requires.<br/>A move to ECS managed instances is planned. | Trusted Fargate workloads.<br/>Use Kubernetes for the full sandbox boundary. |
| macOS or Windows development | None | The `direct` backend lacks the Linux nsjail boundary. | Local development with trusted code only. |

- See [Self-hosted security](/self-hosting/security#isolation) to choose an isolation level and enable nsjail.
