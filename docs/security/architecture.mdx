...
Workspace sync exports workflows, agent presets, skills, tables, case tags, case fields, case dropdowns and case durations (with the `case_addons` entitlement), and variables to a GitHub or GitLab repository you own. Secrets sync as key names only. Values never leave Tracecat.
