...
Tracecat's threat model starts with platform-enforced boundaries:

- Identity, RBAC, and tenant isolation.
- Sandboxed execution for custom code and agents.
- Short-lived authorization tokens and brokered credentials.
- Encrypted durable payloads and object-storage externalization.
- Deterministic tool policy and human approval.
- Versioned change management, Git Ops, audit logs, and agent telemetry.
...
flowchart LR
  ...
  subgraph control["Tracecat control plane"]
    ...
    audit["Platform and organization<br/>user audit events"]
...
Tracecat separates in-scope threats into platform and application threats, and AI threats.
...
- Malicious or compromised tenant users who submit hostile code or workloads.
...
Tracecat treats every tenant workload as potentially hostile. A tenant user may try to escape an executor, reach another tenant, access platform services, steal credentials, or exhaust shared resources.
...
| Human approval | Disguise the purpose of a tool call.<br/>Mislead an approver with incomplete context.<br/>Attempt to resume execution without an authorized decision. | Authenticated approver checks.<br/>Temporal durable execution.<br/>Decision bound to the run and tool call.<br/>Per-tool approval audit event. |
...
### Data protection and durable state

<Badge stroke color="gray" size="sm" shape="pill">A04</Badge>

- Tracecat encrypts workflow and agent inputs and outputs before Temporal stores them.
- The encryption context is scoped to the workspace.
- Large triggers and action results move automatically to object storage.
- Workflow state keeps a scoped object reference instead of copying the full payload through durable history.
- Tracecat encrypts sensitive settings and stored credentials at rest.
...
- Trusted services validate those claims before resolving provider credentials.
- Trusted proxies add credentials to API, LLM, and remote HTTP MCP calls.
...
Tracecat uses token budgets to control inference consumption during hosted-agent runs. It does not use fixed LLM-request or tool-call counts as the primary boundary for long-running agents.

- `Total token budget`: Bounds cumulative LLM token usage for the run. This caps worst-case inference consumption across multi-step work.
- `Token burn-rate limit`: Controls how quickly the run consumes its budget. This contains abnormal loops and cascading LLM calls before the total budget is exhausted.
- `Independent execution controls`: Sandbox wall-time and resource limits continue to contain execution. Tool policy and human approval continue to control side-effect risk.
...
- Custom Python actions and hosted agents run inside nsjail by default on full-isolation profiles.
- Tracecat treats tenant code, third-party packages, and generated output as untrusted.
...
- Tracecat keeps secrets referenced by preset-agent Expressions outside LLM context.
...
- Cover actions performed through the platform administration boundary.
- Include platform settings, users, organizations, tiers, and registry administration.
- Record the authenticated actor, resource, action, result, and request source.
- Stream to a separate platform-scoped HTTPS audit webhook.
- Remain separate from every organization audit sink.

### Organization user audit logs

- Cover authenticated organization users and service accounts.
- Include organization and workspace scope when applicable.
- Record the actor, request source, resource, action, result, and event time.
- Include the approver and decision time for authenticated UI and API approvals.
- Exclude prompts and tool arguments.
- Exclude outputs and overrides.
- Exclude credentials.
- Do not yet include Slack-originated approval actors.
- Stream to the configured [organization audit webhook](/audit-logs/organization).
- Use best-effort delivery so a collector cannot block the audited action.
- Do not create a second stored copy of the webhook stream in Tracecat.

### Organization agent logs

- Export OpenTelemetry logs, traces, and metrics by organization and agent run.
- Record tool activity and the initial Tracecat policy decision.
- Record the later human acceptance or rejection.
- Route telemetry through a scoped OTEL relay.
- Keep observability-backend credentials outside the agent runtime.
- Send telemetry to your OTLP-compatible observability backend.
- Use the retention policy configured in that backend.

### Workspace MCP access logs

- Group OAuth connections and personal access tokens by user.
- Show external Tracecat MCP tool calls.
- Scope this activity to the current workspace.
- Present it in the workspace MCP access page.
- Keep it distinct from platform and organization audit webhooks.
...
- Published workflow definitions are immutable and pin their registry dependencies.
- Test draft changes before publishing a new runtime version.
...
- Revert to a known-good commit and sync it to roll back configuration.
...
- See [Organization audit logs](/audit-logs/organization) to stream organization user audit events.
