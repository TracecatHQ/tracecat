...
A secret either holds its values in Tracecat, encrypted at rest, or references a value that stays in AWS Secrets Manager. See [External secret stores](/automations/core-concepts/external-secret-stores) for the AWS-backed path.
...
- See [External secret stores](/automations/core-concepts/external-secret-stores) for referencing AWS Secrets Manager values instead of storing them in Tracecat.
