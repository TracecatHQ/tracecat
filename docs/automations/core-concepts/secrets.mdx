...
3. The `google_admin` service-account integration, `${{ SECRETS.google_admin_oauth.GOOGLE_ADMIN_SERVICE_TOKEN }}`, with domain-wide delegation through its subject.
4. The `google_api` credential, key `GOOGLE_API_CREDENTIALS`, with domain-wide delegation through `GOOGLE_API_SUBJECT`.
