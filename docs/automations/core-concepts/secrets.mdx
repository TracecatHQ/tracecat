...
The Admin SDK namespaces `google_directory`, `google_reports`, and `google_alert_center` have no per-service integration, so they start at step 3. An integration that is configured but not connected has no token, so the chain moves on to the next source.
