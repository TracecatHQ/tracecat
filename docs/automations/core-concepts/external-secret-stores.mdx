...
Go to Organization settings → Secret stores and select `Add AWS store`. You need
...
After you save, the store card shows the generated external ID along with the
trust policy and permissions policy to copy into AWS. A store reads secrets in one
region only. Create a second store for a second region.

![Store card with external ID, trust policy, and permissions policy](/img/automations/secret-stores/store-policies.png)

Editing a store's name, description, role ARN, region, or enabled flag keeps the
external ID; the external ID is immutable for the life of the store.

## Authorize workspaces

A store is invisible to workspaces until you authorize them. On the store card,
pick a workspace under `Authorized workspaces` and select `Authorize`. The
workspace view of a store shows only its name, description, provider, region, and
enabled flag, never the external ID or role ARN.

Remove an authorization with the `×` on the workspace badge.

![Authorize a workspace on the store](/img/automations/secret-stores/authorize-workspace.png)

## Create an AWS-backed secret

In the workspace, open `/credentials`, select `Create credential`, and switch the
source tab from `Stored in Tracecat` to `AWS Secrets Manager`.
...
Each AWS-backed credential in the inventory shows a `Check` button in place of
`Edit`. It resolves the reference server-side and reports `Reachable` with the
