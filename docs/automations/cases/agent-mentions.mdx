...
Without `agent:read` and `agent:execute`, `@` behaves as regular comment text and never starts an agent run.
...
A mentioned agent can run workflows configured as tools on its preset. When you need fixed order, deterministic branching, or guaranteed steps, use a `/` command to [run the workflow directly](/automations/triggers/comment-triggers).

## Related pages

- See [Case management](/automations/cases) to configure and manage case records.
