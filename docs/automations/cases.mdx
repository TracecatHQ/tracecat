...
description: "Track and resolve Tracecat cases: triage alerts, run agents and workflows on case events, manage tasks and comments, and report on outcomes."
---
...
Mention a preset agent when you want it to investigate a case and reply in a comment thread.
Tracecat gives the agent the case and thread context, posts its final answer as a reply, and keeps the full run in case chat history.

See [Mention agents in case comments](/automations/cases/agent-mentions) to configure access and follow an agent run.
...
Each linked table renders as its own grid in the case's Tables tab. To link rows from the case page:
...
![Link rows dialog](/img/cases/link-rows-dialog.png)
...
See [Case actions](/automations/core-actions/case-actions/cases) to create, fetch, update, search, and delete cases; manage comments and attachments; and use Enterprise tasks, linked rows, and metrics.
