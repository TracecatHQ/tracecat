...
Case custom fields reuse the shared column `type` values `TEXT`, `INTEGER`, `NUMERIC`, `BOOLEAN`, `DATE`, `TIMESTAMPTZ`, `SELECT`, and `MULTI_SELECT`.
The custom field picker also exposes `Long text` and `URL`, which are case-only UI kinds layered on top of `TEXT` and `JSONB`.
