...
Case custom fields reuse the shared values `TEXT`, `INTEGER`, `NUMERIC`, `BOOLEAN`, `DATE`, `TIMESTAMPTZ`, `SELECT`, and `MULTI_SELECT`.
The case field-only picker options `Long text` and `URL` are UI kinds layered on top of `TEXT` and `JSONB`.
