...
A workflow that recreates the table each run must guard the index step, and downstream steps need `join_strategy: any` so the guard being skipped does not skip them too:
