...
A workflow that repeats the index step on every run must guard it, and downstream steps need `join_strategy: any` so a skipped guard does not skip them too:
