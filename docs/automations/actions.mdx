---
title: "Actions"
description: "Learn how Tracecat actions work and how to configure them: action names and types, the action toolbar, success and error paths, and control flow."
---
...
import CoreActionsList from "/snippets/core-actions-list.mdx";
...
An explicit value clamps to between 1800 seconds and `TRACECAT__AGENT_SANDBOX_TIMEOUT`, which defaults to 3600 seconds (see [Environment variables](/self-hosting/environment-variables)). Tracecat stores the clamped value instead of rejecting the action.

The timeout covers active runtime. A pause for a tool approval does not count, and the resumed run gets the full timeout again.
