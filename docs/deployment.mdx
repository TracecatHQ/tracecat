...
The following resources must be created before deploying the Tracecat CDK stack:
- 1 VPC
- 1 AmazonMQ Broker
- 1 Private Subnet for the AmazonMQ Broker
- 1 Security Group attached to the AmazonMQ Broker

You can either do this manually in the AWS console or use the Tracecat managed CDK stack `aws/rabbitmq.py` (coming soon).

Once the AmazonMQ service is up and running. Make sure to note the AmazonMQ URI and the security group ID:
- The AmazonMQ URI is used to connect to the RabbitMQ broker and needs to be added to the AWS secret (see instructions below).
- The security group ID is set as environment variable `RABBITMQ__SECURITY_GROUP_ID` (see instructions below) and is used to allow the ECS Fargate services to connect to the RabbitMQ broker.
...
<Steps>
  <Step title="Create AWS secret">
    ...
    rabbitmq-uri          = your_rabbitmq_uri
