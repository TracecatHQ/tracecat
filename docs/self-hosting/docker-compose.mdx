...
description: "Deploy self-hosted Tracecat using Docker Compose: bring up the API, executor, worker, frontend, Postgres, and Temporal stack on a single host in minutes."
---
...
curl -o env.sh https://raw.githubusercontent.com/TracecatHQ/tracecat/1.1.0-alpha.2.1/env.sh

# 2. Download the .env.example template file (env.sh needs this to generate your .env file)
curl -o .env.example https://raw.githubusercontent.com/TracecatHQ/tracecat/1.1.0-alpha.2.1/.env.example
...
```bash
curl -o Caddyfile https://raw.githubusercontent.com/TracecatHQ/tracecat/1.1.0-alpha.2.1/Caddyfile
```

## Download Docker Compose file

```bash
curl -o docker-compose.yml https://raw.githubusercontent.com/TracecatHQ/tracecat/1.1.0-alpha.2.1/docker-compose.yml
```
...
To size executor replicas, concurrency settings, and PostgreSQL connections for a target of up to 256 actions per second, see [Scaling action throughput](/self-hosting/scaling).
...
<AccordionGroup>
  ...
  <Accordion title="Connecting LiteLLM, vLLM, or Ollama fails with 'Host is not allowed'">
    Tracecat rejects LLM provider and MCP endpoints that resolve to private addresses unless you allow them.
    Set `TRACECAT__OUTBOUND_ALLOWED_PRIVATE_CIDRS` in your `.env` file to the gateway's IP or CIDR, e.g. `TRACECAT__OUTBOUND_ALLOWED_PRIVATE_CIDRS=10.0.12.34/32`, then run `docker compose up -d` to restart the services.
    See [Private endpoints](/agents/llm-providers#private-endpoints).
  </Accordion>
