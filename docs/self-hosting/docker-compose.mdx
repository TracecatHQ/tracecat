...
The default concurrency settings are conservative: one executor with 16 action slots and one worker. See [Scaling and performance tuning](/self-hosting/scaling) for the variables that raise them, the sizing formula, and a calculator script that prints the values for your target throughput.
