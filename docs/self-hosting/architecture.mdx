...
Use PostgreSQL 14 or newer for the Tracecat application database. The bundled
...
<AccordionGroup>
  <Accordion title="Can I use an external PostgreSQL or Redis?">
    Yes. Update `TRACECAT__DB_URI` to point to your managed PostgreSQL 14 or newer instance and `REDIS_URL` for Redis. For Temporal, update the Temporal PostgreSQL credentials in your `.env` file.
