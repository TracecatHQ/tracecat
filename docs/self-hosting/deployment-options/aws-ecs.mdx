...
AWS Fargate deployments have `nsjail` **disabled by default**. In that mode, script execution falls back to `unsafe_pid_executor` PID namespace isolation (when available), which is less isolated than `nsjail`.
