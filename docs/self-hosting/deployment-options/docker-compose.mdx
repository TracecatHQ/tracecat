...
```bash
curl -o env.sh https://raw.githubusercontent.com/TracecatHQ/tracecat/0.8.2/env.sh
...
```bash
curl -o Caddyfile https://raw.githubusercontent.com/TracecatHQ/tracecat/0.8.2/Caddyfile
```
...
- Read inline comments in the generated `.env` file to better understand Tracecat's configurations. [View `.env.example` file](https://raw.githubusercontent.com/TracecatHQ/tracecat/0.8.2/.env.example)
