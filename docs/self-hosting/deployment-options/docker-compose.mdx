...
This self hosting guide will walk you though the steps to self host Tracecat using Docker Compose.
...
```bash
curl -o .env https://raw.githubusercontent.com/TracecatHQ/tracecat/0.6.10/env.sh
curl -o .env https://raw.githubusercontent.com/TracecatHQ/tracecat/0.6.10/env.sh
...
```bash
curl -o .env https://raw.githubusercontent.com/TracecatHQ/tracecat/0.6.10/docker-compose.yml
```
...
- Read inline comments in the generated `.env` file to better understand Tracecat's configurations. [View `.env.example` file](https://raw.githubusercontent.com/TracecatHQ/tracecat/0.6.10/.env.example)
