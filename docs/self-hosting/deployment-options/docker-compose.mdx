...
### "Network error" or "Failed to fetch" in the sign-in page

Your browser cannot connect to the API service.
This happens when the frontend sends API requests to an incorrect or unreachable address.
...
- Docker Desktop users: Use `host.docker.internal` or the Docker VM's IP address instead of localhost.

**Fix environment variables:**
- Set `NEXT_PUBLIC_API_URL` to an address your browser can reach.
- Ensure `PUBLIC_API_URL` matches what your browser can access.

**Check service health:**
- Run `docker compose ps` to verify all services are running.
- Check the temporal service is healthy - worker connectivity issues affect login.
