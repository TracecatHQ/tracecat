...
```bash
curl -o env.sh https://raw.githubusercontent.com/TracecatHQ/tracecat/0.13.1/env.sh
curl -o .env.example https://raw.githubusercontent.com/TracecatHQ/tracecat/0.13.1/.env.example
...
<Note>
You'll be prompted to input the email and password for the super admin user.
...
```bash
curl -o Caddyfile https://raw.githubusercontent.com/TracecatHQ/tracecat/0.13.1/Caddyfile
```

## Download Docker Compose File

```bash
curl -o docker-compose.yml https://raw.githubusercontent.com/TracecatHQ/tracecat/0.13.1/docker-compose.yml
```
...
Every new Tracecat deployment is created with a default super admin user.
The email and password for this user is configured in the `.env` file, which is created when you run the `env.sh` script.
...
Login to Tracecat using the email and password you configured for this super admin user.
You can change the email and password for this user in Tracecat settings after logging in.
...
- Read inline comments in the generated `.env` file to better understand Tracecat's configurations. [View `.env.example` file](https://raw.githubusercontent.com/TracecatHQ/tracecat/0.13.1/.env.example)
