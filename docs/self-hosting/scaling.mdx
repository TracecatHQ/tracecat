...
With the reference workload a replica with 16 admitted activities completes about 6 actions per second at most, and only if it has roughly 20 free cores.
On a 4 vCPU replica, the formula gives `0.8 × 4 × 2.6 / 1.21 ≈ 7` admitted activities; 8 is the validated burst-friendly setting.
...
Every tier uses the same executor replica shape: 4 vCPU, 4 GiB memory, and a Postgres client pool of 8 + 8 connections.
...
| :----- | ----------------: | --------------------------: | ----------------: | :------------------ | :------------------- | -------------------------: | :------- |
| Default settings | 1 | 16 | 16 | Unlimited (shares host) | 8 vCPU / 16 GiB | 100 | ~3–5 actions/s on an 8-core host |
| 8 actions/s | 3 | 8 | 24 | 12 vCPU / 12 GiB | 20 vCPU / 32 GiB | 200 | Measured: 8.4–8.5 actions/s |
| 16 actions/s | 6 | 8 | 48 | 24 vCPU / 24 GiB | 32 vCPU / 48 GiB | 300 | Projected |
| 32 actions/s | 12 | 8 | 96 | 48 vCPU / 48 GiB | 56 vCPU / 80 GiB | 400 | Projected |
| 64 actions/s | 24 | 8 | 192 | 96 vCPU / 96 GiB | 104+ vCPU / 144 GiB | 600 or a pooler | Projected, multi-node |
| 128 actions/s | 48 | 8 | 384 | 192 vCPU / 192 GiB | 200+ vCPU / 256 GiB | Pooler required | Projected, multi-node |
| 256 actions/s | 96 | 8 | 768 | 384 vCPU / 384 GiB | 392+ vCPU / 512 GiB | Pooler required | Projected, multi-node |

Only the 8 actions/s row is measured.
The higher rows keep the validated replica shape fixed (4 vCPU, 8 admitted activities, roughly 2.7 actions per second per replica) and double the replica count for each doubling of throughput, so the scaling rule is three executors per 8 actions per second.
They assume the reference action mix and that every replica gets its own physical cores.
...
A single executor with 16 admitted activities can only reach 16 / 2.6 ≈ 6 actions per second, and on an 8-core host shared with every other service it has roughly 4 cores available, which caps it at 0.8 × 4 / 1.21 ≈ 2.6 actions per second for the reference workload.
Raising `TRACECAT__EXECUTOR_MAX_CONCURRENT_ACTIVITIES` on that host adds queueing and CPU contention, not throughput.
...
<Steps>
  <Step title="Set executor concurrency in .env">
    Set both executor variables to the `MAX_CONCURRENT_ACTIVITIES` value from the table.

    ```bash
    TRACECAT__EXECUTOR_MAX_CONCURRENT_ACTIVITIES=8
    TRACECAT__EXECUTOR_THREADPOOL_MAX_WORKERS=8
    ```

    Actions that use `for_each` multiply database concurrency by `TRACECAT__EXECUTOR_FOR_EACH_MAX_CONCURRENCY` (default 4).
    Keep `MAX_CONCURRENT_ACTIVITIES × FOR_EACH_MAX_CONCURRENCY` below the executor's Postgres pool ceiling, or lower `TRACECAT__EXECUTOR_FOR_EACH_MAX_CONCURRENCY` to 1 for scatter-heavy workloads.
  </Step>
  ...
  <Step title="Verify against the target">
    ...
    - `docker stats`: executor containers should sit below 80% of their CPU limit. Sustained 100% means you need more replicas or fewer admitted activities per replica.
...
From 12 executor replicas upward, place a transaction-mode pooler such as PgDog or PgBouncer between the executors and PostgreSQL, and size its executor backend pool at roughly two-thirds of the total in-flight actions.
