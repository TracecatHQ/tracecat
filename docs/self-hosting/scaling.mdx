...
With the reference workload a replica with the default 32 admitted activities completes about 12 actions per second at most, and only if it has roughly 40 free cores.
...
The default admits bursts generously; adding cores or replicas, not a higher `TRACECAT__EXECUTOR_MAX_CONCURRENT_ACTIVITIES`, raises sustained throughput on that host.
