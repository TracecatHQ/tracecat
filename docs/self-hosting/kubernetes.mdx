...
The chart version and Tracecat application version are separate. Use either of the following methods to update the application.

### Use the chart `appVersion`

Each chart version has an `appVersion`. By default, the chart uses this version for both the backend and UI image tags.
...
Choose a chart whose `appVersion` matches the Tracecat version you want, then upgrade the release:
...
### Override image tags in values

To update Tracecat without changing the chart version, set both image tags in `values.yaml`:
...
Run the same `helm upgrade` command after updating `values.yaml`. If you set `migrations.image.tag` separately, it continues to override `image.tag` for migrations.
