...
- External PostgreSQL 15 or newer instance (e.g., Amazon RDS, Cloud SQL)
...
Use PostgreSQL 15 or newer for the Tracecat application database. [Set up pgvector](/self-hosting/pgvector) before running migrations.
...
<AccordionGroup>
  ...
  <Accordion title="Connecting an in-cluster LiteLLM, vLLM, or Ollama fails with 'Host is not allowed'">
    Tracecat rejects LLM provider and MCP endpoints that resolve to private addresses unless you allow them.
    Set `tracecat.outboundAllowedPrivateCidrs` in your values to the service's cluster IP or CIDR, e.g. `"10.96.0.0/12"`, and upgrade the release.
