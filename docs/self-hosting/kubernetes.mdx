---
title: Kubernetes
description: "Deploy self-hosted Tracecat to Kubernetes with the OCI Helm chart: install Tracecat services and Temporal on EKS, GKE, or any conformant cluster."
---
...
```bash
helm show chart oci://<ecr-registry-url>/tracecat \
  --version <chart-version>
```
...
`image.tag` sets the backend image and the default migrations image. `uiImage.tag` sets the UI image. Both override the chart's `appVersion`; keep them on the same compatible release.

Run the chart upgrade command after updating `values.yaml`. If you set `migrations.image.tag` separately, it continues to override `image.tag` for migrations.

## Security

### Execution sandboxing

The chart enables nsjail by default (`tracecat.sandbox.disableNsjail: false`). nsjail isolates user Python scripts, custom registry actions, and agent runtimes inside the `executor` and `agent-executor` pods.
...
`tracecat.sandbox.disableNsjail: false` is one boolean that fans out to four changes on both the `executor` and `agent-executor` deployments:

| Rendered | Source |
| --- | --- |
| `TRACECAT__DISABLE_NSJAIL` env var, plus fixed nsjail binary, rootfs, and cache paths | `templates/_helpers.tpl` |
| Container `securityContext` with `privileged: true`, `capabilities.add: [SYS_ADMIN]`, and `seccompProfile.type: Unconfined` | `templates/executor-deployment.yaml` |
| `volumeMount` of `dev-tun` at `/dev/net/tun` | `templates/executor-deployment.yaml` |
| `hostPath` volume for `/dev/net/tun`, typed `CharDevice` | `templates/executor-deployment.yaml` |

This is the `helm template` diff of the executor Deployment with the flag toggled — lines added when nsjail is enabled:
...
The `agent-executor` Deployment renders the identical four changes.

Do not confuse the two `securityContext` levels. The pod-level one is always present and comes from values:
...
The container-level one in the diff above appears only when nsjail is enabled and is not configurable.

#### Why privileged and not just SYS_ADMIN

Kubernetes has no equivalent of Docker's `systempaths=unconfined`, which nsjail needs in order to remount `/proc` inside the jail. Privileged containers get an unmasked `/proc` as a side effect, so the chart uses that.

The targeted alternative, `securityContext.procMount: Unmasked`, needs the `ProcMountType` feature gate, and the chart does not expose it. `privileged: true` already implies all capabilities, so the explicit `SYS_ADMIN` add is redundant while privileged is set — dropping `privileged` and keeping `SYS_ADMIN` does not produce a working configuration.

#### Cluster prerequisites

- PodSecurity admission must allow `privileged` in the release namespace. The `baseline` and `restricted` levels both reject `privileged: true` and a non-`Default` seccomp profile, so the pods will not be admitted.
- Every node that can schedule `executor` or `agent-executor` must have `/dev/net/tun`. The volume is typed `CharDevice`, so the pod fails to start on nodes without it — minimal or hardened node images are the usual cause. Pin the deployments with `scheduling.nodeSelector` if only some node groups qualify.

#### Turning nsjail off

Set both the flag and the backends, or it does not take effect:
...
The `ephemeral` backend forces nsjail and bypasses the `TRACECAT__DISABLE_NSJAIL` check entirely. Leaving the default `backend: ephemeral` while setting `disableNsjail: true` still attempts nsjail — now without the security context or the TUN device — so registry actions fail.

The agent runtime reads `TRACECAT__DISABLE_NSJAIL` directly and does fall back, so agents keep working while registry actions fail. The two halves disagreeing is what makes this hard to diagnose.

#### Customizing the profile

The container `securityContext` is a hardcoded literal in the chart templates. Values expose only the global pod-level `securityContext` (`fsGroup: 1001`, `runAsUser: 1001`); there is no `executor.securityContext` key.

A tighter profile — a `Localhost` seccomp profile, or `procMount: Unmasked` instead of `privileged` — requires a post-renderer, Kustomize, or a chart fork.

See [Security](/self-hosting/security#isolation) for isolation levels, coverage by code path, and how to verify what you are running.
