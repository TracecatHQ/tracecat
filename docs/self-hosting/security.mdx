...
| `USER_AUTH_SECRET` | Password reset tokens, email verification, and OAuth state. |
...
- `TEMPORAL__PAYLOAD_ENCRYPTION_KEYRING` rotates cleanly. Add a new key to the keyring and point `current_key_id` at it. New payloads use the new key while existing histories stay readable under the key they were written with, so keep retired keys in the keyring for as long as their histories are retained.
- `TRACECAT__SIGNING_SECRET` can be rotated, but doing so invalidates every existing webhook URL. Plan to reissue them.
- `TRACECAT__SERVICE_KEY` and `USER_AUTH_SECRET` can be rotated with a restart. In-flight password reset and verification links issued under the old value stop working.
