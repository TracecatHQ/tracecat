...
<Steps>
  ...
  <Step title="Create a Compose override">
    ...
    services:
      cert-init:
        image: ghcr.io/tracecathq/tracecat:${TRACECAT__IMAGE_TAG:-1.0.0-beta.52-rc.22.post1}
        restart: "no"
        user: root
        volumes:
          - ./certs:/certs:ro
...
<Note>
  The injected bundle does not reach actions running inside the [nsjail sandbox](/self-hosting/security#isolation) (`TRACECAT__EXECUTOR_SANDBOX_ENABLED=true`).
