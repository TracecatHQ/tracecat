...
<Steps>
  <Step title="Download latest environment variables migration script and template">
    ...
    ```
    curl -o env-migration.sh https://raw.githubusercontent.com/TracecatHQ/tracecat/0.7.1/env-migration.sh
...
As of version 0.7.1, data migrations are automatically applied when the API service is spun up.
