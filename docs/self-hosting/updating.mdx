...
<Steps>
  <Step title="Download latest environment variables migration script and template">
    ...
    version. ``` curl -o env-migration.sh
    https://raw.githubusercontent.com/TracecatHQ/tracecat/0.8.3/env-migration.sh
    curl -o .env.example
    https://raw.githubusercontent.com/TracecatHQ/tracecat/0.8.3/.env.example ```
  </Step>
  <Step title="Execute environment variables migration">
    Run the environment variables migration script to update your existing
    `.env` file with any new or changed variables. ``` chmod +x env-migration.sh
    && ./env-migration.sh ```
  </Step>
  <Step title="Update Docker Compose Configuration">
    Download the latest Docker Compose file. ``` curl -o docker-compose.yml
    https://raw.githubusercontent.com/TracecatHQ/tracecat/0.8.3/docker-compose.yml
    ```
  </Step>
