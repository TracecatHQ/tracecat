...
If you need the highest isolation for untrusted code execution (like custom registry actions or `core.script.run_python`), deploy Tracecat on Kubernetes using our [Helm chart](/self-hosting/kubernetes), where `nsjail` is enabled by default.
