...
If the bundled `temporalio/auto-setup` service is unable to connect to PostgreSQL with TLS in ECS Fargate, you can disable SSL enforcement on the Temporal RDS instance only:
...
This setting applies only to the Temporal RDS instance. It requires a DB reboot and permits non-TLS connections to the Temporal database for compatibility with the current Fargate Temporal auto-setup flow.
