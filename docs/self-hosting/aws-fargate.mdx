...
<AccordionGroup>
  <Accordion title="Is untrusted code execution safe on Fargate?">
    No. Fargate cannot grant the kernel capabilities nsjail requires, so Tracecat runs without isolation on Fargate: the deployment hardcodes the `direct` backend with nsjail disabled.
    Run trusted code only, or [deploy Tracecat on Kubernetes](/self-hosting/kubernetes) for untrusted code such as custom registry actions or `core.script.run_python` — nsjail is enabled there by default.
  </Accordion>
