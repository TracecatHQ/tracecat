...
Webhooks allow Tracecat Workflows to interact with external systems by listening for and responding to events.
  ...
  - type: webhook
    ...
    filters:
      - kind: all # all conditions must be satisfied
        conditions:
          - ${{ FNS.not_null(TRIGGER.value) }}
          - ${{ FNS.equals(TRIGGER.value, "123456") }}
      - kind: any # any condition must be satisfied
        conditions:
          - ${{ FNS.not_null(TRIGGER.value) }}
          - ${{ FNS.contains([1, 2, 3], TRIGGER.value) }}
```
