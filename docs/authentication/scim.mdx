...
The `Group mappings` table lists pushed IdP groups, one row per group. The Connection card shows the directory's user, inactive-user, and group counts.

1. Find your IdP group in the table. Use `Load more groups` when available; `Search groups` filters the groups already loaded.
2. Open the dropdown in the `Tracecat group` column and select one or more target groups. Select a checked group again to remove that mapping.
