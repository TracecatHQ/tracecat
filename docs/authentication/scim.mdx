...
After activation, Tracecat applies IdP changes as they arrive. Each new mapping is reviewed before Tracecat applies it.
...
- Revoke token: stops all provisioning. Existing mappings stay in place but stop receiving updates. Rotate the token to resume.
