...
Tracecat requires signed assertions and signed responses by default.
...
Tracecat requires signed assertions and signed responses by default, and fails
closed on startup if both checks are disabled.
