...
At least one of `SAML_SIGNED_ASSERTIONS` or `SAML_SIGNED_RESPONSES` must remain enabled when SAML auth is enabled. Tracecat fails closed on startup if SAML SSO is enabled while both settings are disabled.
