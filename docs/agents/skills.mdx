...
`Tools` selection. The agent's effective tool set is the union of the preset's actions and
the tools declared by every attached skill.
...
<Note>
  ...
  is dropped from the run and reported under `tool_policy.blocked_tools`. MCP tools a
  skill declares are not filtered by `namespaces`.
</Note>
...
- Preset `actions`: tools that only this one agent needs.
- Skill `metadata.tools`: a group of tools that belong together and are reused across
  several agents. Attaching the skill brings the instructions and the tools together.
- The `actions` argument on the `ai.preset_agent` workflow action: replaces the preset's
  ...
  skill contributed are dropped for that run. MCP tools from the preset and its skills
  stay attached, and an empty list changes nothing. It fits a test or an eval that needs a
  different tool set; the preset or a skill is the better home for tools the agent should
  have on every run, including in chat.
