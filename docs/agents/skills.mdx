...
<Note>
  If the preset restricts `namespaces`, that filter also applies to skill tools.
  A skill tool outside the allowed namespaces is silently dropped from the run and
  reported under `tool_policy.blocked_tools`.
</Note>
...
- The `actions` argument on the `ai.preset_agent` workflow action: ad hoc testing and
  evals only. It replaces the preset's and its skills' entire tool set for that run rather
  than adding to it, so it should never be left in a workflow that ships.
