...
Tracecat rejects base URLs that resolve to private, loopback, or link-local addresses by default. To run Ollama, vLLM, or a LiteLLM proxy on the same network as a single-tenant deployment, set `TRACECAT__AGENT_ALLOW_PRIVATE_LLM_HOSTS=true` on the `api` service. See [Custom LLM providers](/agents/custom-llm-providers#troubleshooting).
