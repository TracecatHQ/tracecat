...
Ollama, vLLM, and LiteLLM base URLs must include the `/v1` segment and be reachable from the Tracecat services. Inside a container, `localhost` points at the container itself, not your host. Base URLs that resolve to a private address also require `TRACECAT__OUTBOUND_ALLOWED_PRIVATE_CIDRS`; see [Private endpoints](#private-endpoints).
...
| --- | --- |
| Docker Compose | `TRACECAT__OUTBOUND_ALLOWED_PRIVATE_CIDRS=10.0.12.34/32` in `.env`; the Compose file passes it to every service that needs it |
| Kubernetes | `tracecat.outboundAllowedPrivateCidrs: "10.0.12.34/32"` in your Helm values |
| AWS Fargate | `outbound_allowed_private_cidrs = "10.0.12.34/32"` in your Terraform variables |

The allowlist applies to every workspace and also covers remote MCP servers, so list only endpoints you operate. Container hostnames such as `litellm` or `vllm` resolve to addresses on the Docker Compose `core` network; run `docker compose exec api getent hosts <hostname>` to find the address to allow, or allow that network's subnet from `docker network inspect`.
