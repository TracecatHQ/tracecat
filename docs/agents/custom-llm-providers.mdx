...
Discovery confirms that the gateway exposes a model ID. It does not confirm that the gateway has local weights or can serve the model without internet access. For an air-gapped deployment, stage every model artifact and send a representative inference request after you remove egress. Ollama models tagged `:cloud` require the Ollama cloud service and cannot run air-gapped.
...
Test connection or Refresh that fails with `Host is not allowed` means the base URL resolves to a private, loopback, or link-local address. Tracecat blocks these by default. Add the gateway's IP or CIDR to `TRACECAT__OUTBOUND_ALLOWED_PRIVATE_CIDRS` on the Tracecat services and restart them. See [Private endpoints](/agents/llm-providers#private-endpoints) for the Docker Compose, Kubernetes, and Fargate settings.

A run that fails with `Model response timed out after <duration> without receiving data` (or `Gateway timeout` on a non-streaming request) hit `TRACECAT__LLM_PROXY_READ_TIMEOUT`, the read timeout between the sandbox LLM proxy and the gateway. The variable takes seconds, defaults to `600`, and applies to both passthrough and managed routes.

A run that fails with `Agent execution timed out after 1800s` hit the action's `timeout`, which caps active runtime at 1800 seconds when unset. Tracecat clamps explicit values between 1800 seconds and `TRACECAT__AGENT_SANDBOX_TIMEOUT` (default `3600`), and a ceiling below 1800 seconds lowers both the default and the floor to the ceiling. See [Actions](/automations/actions#timeout).
...
- See [Environment variables](/self-hosting/environment-variables) for the agent and proxy timeout settings and the private endpoint allowlist.
