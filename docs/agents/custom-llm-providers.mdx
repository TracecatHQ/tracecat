...
A save, test connection, or refresh that fails with `Provider base URL is not allowed` points at a host that resolves to a private, loopback, or link-local address. Tracecat rejects these by default so members cannot steer server-side requests at internal services. On a single-tenant deployment that runs the gateway on the same network, set `TRACECAT__AGENT_ALLOW_PRIVATE_LLM_HOSTS=true` on the `api` service and restart it.
