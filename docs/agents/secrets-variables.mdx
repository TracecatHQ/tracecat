...
<Warning>
**Only use `${{ SECRETS... }}` and `${{ VARS... }}` in preset-stored instructions** — the instructions saved in the agent preset configuration, not in workflow action args.

All workflow action args (`user_prompt`, per-run `instructions` overrides) are evaluated before the LLM call. In those fields, secret expressions resolve to `None` (breaking tool calls) and variable expressions are expanded inline (exposing values to the model). This applies to `ai.action`, `ai.agent`, **and** the `ai.preset_agent` action args.
</Warning>
