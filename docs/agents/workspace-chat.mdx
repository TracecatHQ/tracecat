...
`Agent presets` in `Active capabilities` lets the chat run your saved presets as tools.

`MCP integrations` lists the servers your workspace connected, including catalog connectors, and `Connect an MCP server` adds one; see [MCP servers](/automations/integrations/mcp-integrations) to connect a catalog, remote, or local server. Your workspace role needs `integration:read` to see the section.
