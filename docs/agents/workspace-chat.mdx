...
Focus the empty composer to show the `@ Mention agent` hint, then type `@` and select a preset agent from the list. Selecting an agent sets that preset for the entire conversation, and the `@Agent name` text remains in the message you send.
