...
<CodeGroup>
```bash VirusTotal
curl -X PUT  "$TRACECAT__PUBLIC_RUNNER_URL/secrets" \
  -H "Authorization: Bearer super-secret-jwt-token" \
  -H "Content-Type: application/json" \
  -d '{"type":"custom", "name":"virustotal", "keys":[{"key":"VT_API_KEY", "value": "your-virustotal-api-key"}] }'
...
curl -X PUT  "$TRACECAT__PUBLIC_RUNNER_URL/secrets" \
  -H "Authorization: Bearer super-secret-jwt-token" \
  -H "Content-Type: application/json" \
  -d '{"type":"custom", "name":"fake_api_key", "keys":[{"key":"RESEND_API_KEY", "value": "your-resend-api-key"}] }'
...
curl -X GET "$TRACECAT__PUBLIC_RUNNER_URL/workflows/$WORKFLOW_ID/webhook" -H "Authorization: Bearer super-secret-jwt-token"
```

```bash Trigger the workflow
# Replace $WEBHOOK_URL with the actual URL
