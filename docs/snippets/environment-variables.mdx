...
| `TRACECAT__BLOB_STORAGE_PRESIGNED_URL_ENDPOINT` | | Public base URL substituted for `TRACECAT__BLOB_STORAGE_ENDPOINT` in presigned URLs. Leave unset for AWS S3. |
| `TRACECAT__CSP_CONNECT_SRC_ORIGINS` | | Space- or comma-separated origins added to the UI's CSP `connect-src`. Set on the `ui` service to the presigned S3 bucket origins when the browser uploads or fetches blobs cross-origin. |
