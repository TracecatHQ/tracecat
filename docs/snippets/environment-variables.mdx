...
| `TRACECAT__EXECUTOR_SANDBOX_ENABLED` | `false` | Run registry actions in the nsjail sandbox on the `direct` backend. Independent of `TRACECAT__DISABLE_NSJAIL`. |
| `TRACECAT__SANDBOX_DEFAULT_TIMEOUT` | `300` | Timeout in seconds for sandboxed action execution. |
| `TRACECAT__SANDBOX_DEFAULT_MEMORY_MB` | `2048` | Memory limit in MB for sandboxed action execution. |
| `TRACECAT__AGENT_SANDBOX_TIMEOUT` | `1800` | Timeout in seconds for the agent sandbox runtime. |
| `TRACECAT__AGENT_SANDBOX_MEMORY_MB` | `4096` | Memory limit in MB for the agent sandbox runtime. |
