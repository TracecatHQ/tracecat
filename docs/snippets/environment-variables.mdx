...
| `TRACECAT__BLOB_STORAGE_PRESIGNED_URL_ENDPOINT` | (empty) | Public base URL substituted for `TRACECAT__BLOB_STORAGE_ENDPOINT` in presigned URLs. Compose sets it to `${TRACECAT__PUBLIC_APP_URL}/s3`, which Caddy proxies to MinIO. Leave unset for AWS S3. |
| `TRACECAT__CSP_CONNECT_SRC_ORIGINS` | (empty) | Space-separated origins added to the UI's CSP `connect-src`. Set to the presigned S3 bucket origins (for example `https://<bucket>.s3.<region>.amazonaws.com`) when the browser uploads or fetches blobs cross-origin. Leave unset when presigned URLs share the app origin. |
