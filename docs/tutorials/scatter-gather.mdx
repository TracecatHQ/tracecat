...
The gather action itself always returns `null`.
...
<Steps>
  ...
  <Step title="Use gathered output">
    Because gather returns `null`, add a downstream action to consume the gathered values.

    Use the downstream action's output as the final result for this branch (instead of reading `ACTIONS.gather.result` directly).
  </Step>
