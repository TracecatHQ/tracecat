...
For security, we only support a limited set of core Python features.
...
<Warning>
  This action is for simple oneline Python functions only.
  If you want to use custom Python code in Tracecat,
  we recommend syncing these functions into Tracecat as [Python UDF actions](/integrations/python-udfs).

  Check out more in the [custom integrations tutorial](/tutorials/custom-integrations).
</Warning>
