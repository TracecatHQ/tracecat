...
<Steps>
  <Step title="Open organization settings">
    Open the workspace menu, select **Organization**, then open **Agent OTel**
    under **Settings**.
  </Step>
  <Step title="Set the endpoint">
    Enter the OTLP endpoint of your collector, such as
    `https://otel.example.com`. Tracecat rejects the configuration if you enable
    telemetry with a signal turned on and no endpoint.
  </Step>
  <Step title="Choose signals and headers">
    Keep `metrics_enabled` and `logs_enabled` on for both signals, or turn either
    off. Turn on `traces_enabled` to also export traces. Add any exporter headers
    your collector requires, then save.
  </Step>
</Steps>

You can also read and write the same configuration through `GET /settings/agent-otel` and `PATCH /settings/agent-otel`.

### Exporter headers

Most collectors authenticate the exporter with a header, such as `Authorization: Bearer <collector-token>`. Supply headers as a name-to-value map:

```json
{
  "Authorization": "Bearer <collector-token>",
  "X-Scope-OrgID": "tracecat"
}
```

Tracecat encrypts headers at rest and attaches them to outbound telemetry in the OTLP gateway, outside the agent sandbox, so sandboxed agent code never reads a collector credential. Omitting `agent_otel_headers` from an update leaves the stored headers unchanged; send an empty object to clear them.
