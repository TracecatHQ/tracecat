...
- `metrics_include_session_id` attaches the Claude Code session identifier to metrics. This is Claude Code's own identifier, not the Tracecat session ID; correlate on `tracecat.session.id` instead.
...
- `tracecat.session.id` is the agent session ID, the same value an agent action returns in `ACTIONS.<ref>.result[0].session_id`.
- `tracecat.workspace.id` is the workspace that ran the agent.
- `tracecat.organization.id` is the organization that owns the telemetry configuration.
- `tracecat.user.id` is the Tracecat user or service identity that started the run, when the run has one.
