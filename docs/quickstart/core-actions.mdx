...
<Note>
  ...
  For information on `core.transform`, `core.workflow`, and `core.require`, check out the tutorials on [Data Transforms](/tutorials/data-transforms), [Child Workflows](/tutorials/child-workflows), and [Wait until](/tutorials/wait-retry-until).
</Note>
...
| `core.require` | `Require` | Check if multiple conditions are met. |
