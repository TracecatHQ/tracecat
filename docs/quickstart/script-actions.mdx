...
Without `nsjail`, execution falls back to the `unsafe_pid_executor` subprocess runtime.
This runtime uses PID namespace isolation **only when the host allows `unshare --pid`**.

On AWS Fargate, `unshare --pid` is typically not permitted, so scripts run in subprocess isolation without PID namespace isolation.
In this mode:

- parent process environment variables are not inherited by default (only explicit `env_vars` are passed)
- `allow_network` is best-effort only and is **not** OS-enforced
- isolation is weaker than `nsjail` (no mount/network/cgroup sandboxing)

For production deployments with untrusted code, enable `nsjail` before running script actions.

- `docker compose` deployments have `nsjail` disabled by default
- AWS Fargate deployments have `nsjail` disabled by default and usually cannot use PID namespaces
