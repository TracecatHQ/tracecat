...
Without `nsjail`, execution falls back to the `unsafe_pid_executor` subprocess runtime, which uses PID namespace isolation when available.
This is stronger than AST filtering, but it is still not equivalent to full `nsjail` isolation (no mount/network cgroup controls).
...
- AWS Fargate does not provide the required permissions model for `nsjail`; fallback PID isolation works there, but Kubernetes (Helm chart) is still recommended for highest isolation
