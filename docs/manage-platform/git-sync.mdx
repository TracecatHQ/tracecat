...
For GitHub, `Connect` opens the `Connect GitHub App` dialog with two tabs. `Create new` takes an organization name and a GitHub host URL, and `Create GitHub App` sends you through GitHub's manifest flow; GitHub redirects back to Tracecat, which stores the app credentials and forwards you to GitHub's installation page.
...
The API equivalents are `POST /organization/vcs/github/credentials` and `POST /organization/vcs/gitlab/credentials`, each with `GET .../credentials/status` and `DELETE .../credentials` alongside. Saving requires `org:settings:update`.
...
A full export writes `tracecat.json` and one directory per resource type: `workflows`, `agent_presets`, `skills`, `tables`, `case_tags`, `case_fields`, `case_dropdowns`, `case_durations`, `variables`, and `secret_metadata`. Each workflow lands at `workflows/<id>/definition.yml`.
...
`Preview changes` projects the export and diffs it against the target branch without writing. `Push & open PR` (or `Update branch & open PR` for an existing branch) commits and opens a pull request into the base branch; GitLab connections label these `MR`.

Tracecat commits only files whose content changed and deletes stale files under the exported directories. When nothing changed, the response reports `no_op`, and an open pull request from the same branch is reused rather than duplicated.

The pull request body reads `Automated workspace sync from Tracecat` with the workspace name and the publishing user's email. Schedules stay out of exports from this tab.
...
The API is `POST /workflows/sync/export` with `message`, `branch`, `create_pr`, optional `pr_base_branch`, optional `resources`, and `include_schedules`. `POST /workflows/sync/export/preview` takes `resources`, `include_schedules`, and `compare_ref` and returns the projected files without writing; both require `workspace_sync:sync`, which `workspace-editor` and `workspace-admin` hold.
...
Every pull names one commit SHA; a request without `commit_sha` fails validation. Tracecat pulls only when you or your pipeline call it, so a merged pull request changes nothing in a workspace until someone pulls that commit.
