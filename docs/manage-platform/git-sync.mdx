...
A full export writes `tracecat.json` and one directory per resource type: `workflows`, `agent_presets`, `skills`, `tables`, `case_tags`, `case_fields`, `case_dropdowns`, `case_durations`, `variables`, and `secret_metadata`. The `case_dropdowns` and `case_durations` directories are written only when the organization holds the `case_addons` entitlement. Each workflow lands at `workflows/<id>/definition.yml`.
