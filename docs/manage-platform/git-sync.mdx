...
Sync endpoints require `workspace_sync:sync`, and a service account with the scope can pull from CI. Credential endpoints require `org:settings:update`.

| Action | Endpoint |
| --- | --- |
| Save provider credentials | `POST /organization/vcs/{provider}/credentials` (`github` or `gitlab`) |
| Bind a repository | `PATCH /workspaces/{workspace_id}` with `settings.git_provider` and `settings.git_repo_url` |
| Preview an export | `POST /workflows/sync/export/preview` |
| Export and open a pull request | `POST /workflows/sync/export` |
| Push one workflow | `POST /workflows/{workflow_id}/publish` |
| List branches and commits | `GET /workflows/sync/branches`, `GET /workflows/sync/commits?branch=` |
| Pull a commit | `POST /workflows/sync/pull` with a required `commit_sha` |
