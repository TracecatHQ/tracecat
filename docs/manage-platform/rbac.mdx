...
| `workspace-admin` | Workspace | Everything `workspace-editor` can, plus delete those resources, update or delete the workspace, invite, update, and remove members, and manage workspace service accounts. | 54 |
...
| `organization-admin` | Organization | Everything `workspace-admin` can in every workspace, plus create workspaces, manage organization settings, members and invitations, roles, groups, and assignments, organization service accounts, the custom registry, and organization secrets, and read billing. | 82 |
| `organization-owner` | Organization | Everything `organization-admin` can, plus `org:delete`, `org:billing:update`, and `org:owner:assign`. | 85 |
...
Any role in the organization, held directly or through a group, makes the user an organization member. Workspace resource access is separate: it needs a workspace role, or `organization-admin` or `organization-owner`, both of which reach every workspace in the organization without a workspace membership.
