...
An accepted organization invitation creates the membership and an organization-wide assignment of the invited role. An accepted workspace invitation creates the organization membership when the user has none, then the workspace role assignment. Membership alone carries baseline organization access, with no organization-wide role assigned. See [User management](/manage-platform/users) for invitations.
