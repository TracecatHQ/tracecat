...
An accepted owner or admin invitation creates membership and an organization-wide assignment of the invited role. When you invite an organization member from the platform admin console, acceptance creates membership alone. An accepted workspace invitation creates the organization membership when the user has none, then the workspace role assignment. Membership alone carries baseline organization access, with no organization-wide role assigned. See [User management](/manage-platform/users) for invitations.
