...
For example, the following expression will first evaluate the expression `FN.join(["hello", "world"], " ")` as `hello world`,
then call `FN.contains` with the arguments `hello world` and `["hello", "world", "tracecat"]`, and finally return the boolean value `true`:
