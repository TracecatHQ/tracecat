...
| `INPUTS`  | `INPUTS.<jsonpath>`                 | Reference static workflow inputs        |
...
Static inputs are fixed values defined in the workflow settings. They are accessed using the `INPUTS` prefix:
...
![Static Inputs](/images/static-inputs.png)
